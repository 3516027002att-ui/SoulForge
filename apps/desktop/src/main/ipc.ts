...
export function registerIpcHandlers(webContents: WebContents, rendererDocumentUrl: string): void {
  ...
  // Workspace domain handlers moved to ipc/workspace.ts (A12).
  // Composition root delegates via TrustedIpcHandle; no ipcMain in domain.
  // resource.preview / resource.search / resource.saveText remain here:
  // they are generic resource handlers, not workspace-lifecycle specific
  // (A12.1 – ambiguous → keep with composition-root comment).
    ...
    return {
      ...
      ...(summary.latestSession
        ? {
            latestSession: {
              ...summary.latestSession,
              active: isAgentSessionActive(summary.latestSession.sessionId)
            }
          }
        : {})
    };
