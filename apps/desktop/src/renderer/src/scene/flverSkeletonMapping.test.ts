...
describe('FLVER skeleton mapping', () => {
  ...
  it('retargets animation deltas while preserving a follower bind pose', () => {
    ...
    const follower: FlverRetargetBone[] = [
      ...
      { name: 'Ctrl', parentIndex: 1, translation: [0, 20, 0], rotation: [0, 0, 0], scale: [3, 4, 5] }
    ];
    ...
    assert.ok(Math.abs((pose[2]?.rotation[2] ?? 0) - Math.SQRT1_2) < 1e-12);
    assert.ok(Math.abs((pose[2]?.rotation[3] ?? 0) - Math.SQRT1_2) < 1e-12);
  });
});
