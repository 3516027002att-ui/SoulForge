...
export interface MapGeometryPrepareDiagnostic {
  severity: 'warning';
  code: 'MAP_STATIC_GEOMETRY_CULL_METADATA_AMBIGUOUS';
  message: string;
  details?: {
    chunkId?: string;
    selectedFaceSetCount?: number;
    cullFlagCount?: number;
  };
}

export interface MapMaterialGroup {
  start: number;
  count: number;
  /** Native FLVER material index; renderer variant slots are derived later. */
  materialIndex: number;
  /** Full source FaceSet provenance; never select the first item on ambiguity. */
  faceSetOrdinals?: number[];
...
export interface MapStaticGeometryChunk {
  chunkId?: string | null;
  sourceTriangleStart?: number | null;
  triangleCount?: number | null;
  selectedFaceSetOrdinals?: number[] | null;
  faceSetCullBackfaces?: boolean[] | null;
...
export type MapMeshGeometryData = Omit<Partial<MeshGeometryWire>, 'positionsBase64' | 'materialGroups' | 'diagnostics'> & {
  ...
  diagnostics?: MapGeometryPrepareDiagnostic[];
};
...
const MAX_CULL_METADATA_DIAGNOSTICS = 8;

function isNonNegativeInteger(value: unknown): value is number {
  return typeof value === 'number' && Number.isInteger(value) && value >= 0;
}

function recordCullMetadataDiagnostic(
  diagnostics: MapGeometryPrepareDiagnostic[],
  chunk: MapStaticGeometryChunk,
  selectedFaceSetCount: number,
  cullFlagCount: number
): void {
  if (diagnostics.length >= MAX_CULL_METADATA_DIAGNOSTICS) return;
  diagnostics.push({
    severity: 'warning',
    code: 'MAP_STATIC_GEOMETRY_CULL_METADATA_AMBIGUOUS',
    message: 'FaceSet 背面剔除元数据不完整或包含多个候选；该 draw group 保守使用 unknown 双面材质。',
    details: {
      ...(typeof chunk.chunkId === 'string' && chunk.chunkId.length > 0 ? { chunkId: chunk.chunkId } : {}),
      selectedFaceSetCount,
      cullFlagCount
    }
  });
}

function resolveChunkFaceSetMetadata(
  chunk: MapStaticGeometryChunk,
  groupCount: number,
  chunkVertexCount: number,
  allHaveIndices: boolean,
  diagnostics: MapGeometryPrepareDiagnostic[]
): Pick<MapMaterialGroup, 'faceSetOrdinals' | 'faceSetCullBackfaces' | 'cullBackfaces'> {
  const rawOrdinals = chunk.selectedFaceSetOrdinals;
  const rawCullFlags = chunk.faceSetCullBackfaces;
  const hasCullMetadata = rawOrdinals !== undefined || rawCullFlags !== undefined;
  if (!hasCullMetadata) return {};

  const faceSetOrdinals = Array.isArray(rawOrdinals) ? [...rawOrdinals] : [];
  const faceSetCullBackfaces = Array.isArray(rawCullFlags) ? [...rawCullFlags] : [];
  const sourceStart = chunk.sourceTriangleStart;
  const sourceCount = chunk.triangleCount;
  const decodedElementCount = allHaveIndices ? groupCount : chunkVertexCount;
  const spanMatchesDecodedChunk = isNonNegativeInteger(sourceStart)
    && isNonNegativeInteger(sourceCount)
    && sourceCount > 0
    && decodedElementCount % 3 === 0
    && sourceCount === decodedElementCount / 3;
  const exactlyOneValidFaceSet = faceSetOrdinals.length === 1
    && faceSetCullBackfaces.length === 1
    && isNonNegativeInteger(faceSetOrdinals[0])
    && typeof faceSetCullBackfaces[0] === 'boolean'
    && spanMatchesDecodedChunk;

  if (!exactlyOneValidFaceSet) {
    recordCullMetadataDiagnostic(
      diagnostics,
      chunk,
      faceSetOrdinals.length,
      faceSetCullBackfaces.length
    );
    return {
      ...(Array.isArray(rawOrdinals) ? { faceSetOrdinals } : {}),
      ...(Array.isArray(rawCullFlags) ? { faceSetCullBackfaces } : {})
    };
  }

  const cullBackfaces = faceSetCullBackfaces[0];
  return {
    faceSetOrdinals,
    faceSetCullBackfaces,
    ...(typeof cullBackfaces === 'boolean' ? { cullBackfaces } : {})
  };
}
...
export function mergeMapStaticGeometryChunks(
  ...
  for (const chunk of geometryChunks) {
    ...
    if (groupCount > 0) {
      ...
      materialGroups.push({
        ...
        materialIndex,
        ...faceSetMetadata,
        ...(typeof chunk.chunkId === 'string' && chunk.chunkId.length > 0
          ? { sourceChunkId: chunk.chunkId }
          : {}),
        ...(typeof chunk.sourceTriangleStart === 'number' && Number.isFinite(chunk.sourceTriangleStart)
          ? { sourceTriangleStart: chunk.sourceTriangleStart }
          : {}),
        ...(typeof chunk.triangleCount === 'number' && Number.isFinite(chunk.triangleCount)
          ? { sourceTriangleCount: chunk.triangleCount }
          : {})
      });
    }
  ...
  if (diagnostics.length > 0) merged.diagnostics = diagnostics;
  return merged;
}
...
export function prepareMapStaticGeometryChunks(
  ...
  const geometry: MeshGeometryWire = {
    ...
    ...(merged.diagnostics ? { diagnostics: merged.diagnostics } : {}),
