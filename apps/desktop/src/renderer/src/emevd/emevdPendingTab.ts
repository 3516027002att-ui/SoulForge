 ...
 * envelope 投影（128 事件 / 每事件最多 64 条指令），`readEmevdFullDocument`
 * 拿权威源码。前者产出的 renderer 文档只被用在两处：sourceHash 标量，和
 * gutter 的「未知指令条数」。而它的 `instructionsSample` 默认只覆盖前 256 条
 * 指令（Bridge `read-emevd-document` 的 instructionPageSize 默认值），
 * `mapEmevdEnvelopeToDocument`（已随 S18-B 删除，函数名只作历史追溯用）对采样
 * 不到的指令一律记 `unknown: true` —— 真实 common.emevd 有 33266 条指令，于是
 * 第 256 条之后的事件全被标成「整段未知」。那次读付了一整趟 Bridge 往返，
 * 换回来的是一份系统性错误的判据。
 *
...
export interface EmevdFullDocumentResponseLike {
  ...
  outline?: EmevdDocumentOutline | null;
}
...
export function eventWarningRowsFromOutline(
  outline: EmevdDocumentOutline | null | undefined
...
export function emevdPendingTabFromFullDocument(
  ...
  const diagnostics: EventSourceTabData['document']['diagnostics'] = [{
    severity: 'info',
  ...
  const document: EventSourceTabData['document'] = {
    schemaVersion: 1,
