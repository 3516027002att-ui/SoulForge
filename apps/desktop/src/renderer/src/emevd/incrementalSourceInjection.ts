 ...
 *
 * 上限是前缀长度（400 行）：阈值一旦接近前缀，打开首帧的挂载 measure 就会把
 * 「视口还在文档顶部」判成近底，未滚动先拉一片 —— 违背 event-common-load.md
 * §3.2「首帧只有 400 行前缀，用户滚到靠近已加载底部再拉下一片」。300 与顶部
 * 视口（底行 ≈ 第 50 行，剩余 ≈ 350 行）仍留有余量。
 *
 * 取 300 而不是更小的 200：单片已提到 1200 行（SOURCE_SLICE_LINES），更大的
 * 提前量让续载 IPC（~几十 ms）在快速滚动下也来得及落地；落地后若视口仍近底，
 * 面板还会链式预取下一片（见 fillOneSliceGuarded），两道一起消除快滚空白。
 */
export const INCREMENTAL_NEAR_BOTTOM_LINES = 300;
