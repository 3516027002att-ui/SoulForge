...
async function openParamContainer(window) {
  ...
  // 侧栏也不再渲染 .file-item（顶层只剩语义领域逻辑库，非领域资源走 Ctrl+K）。
  // PARAM 顶栏入口即打开 GameParam 逻辑库工作台；restoreLastShellState 可能把
  // activeDomain 留在 param 但 sourceUri 留在非 PARAM 文件（重复点 param 是
  // no-op），先切去文本再切回 PARAM 强制挂工作台。
