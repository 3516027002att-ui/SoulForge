    ...
    public sealed class Function
    {
        ...
        public void ComputeGlobalLiveness(HashSet<Identifier> allRegisters)
        {
            ...
            void BitSetFromSet(BitSetArray.BitSet target, HashSet<Identifier> set, bool invert = false)
            {
                ...
                {
                    if (identifierToId.TryGetValue(identifier, out var identifierId))
                        target[identifierId] = !invert;
                }
            }
            ...
            foreach (var block in BlockList)
            {
                foreach (var phi in block.PhiFunctions)
                {
                    ...
                    {
                        if (identifierToId.TryGetValue(def, out var defId))
                            killedIdentifiers.Set(block.BlockIndex, defId, true);
                    }
                }
                ...
                foreach (var inst in block.Instructions)
                {
                    if (inst is not PhiFunction)
                    {
                        ...
                        {
                            /* Some VM instructions expose a register value
                             * without a preceding SSA definition.  It is not
                             * part of this analysis set; treating it as
                             * externally supplied avoids corrupting the
                             * liveness proof or throwing from dictionary
                             * lookup. */
                            if (!identifierToId.TryGetValue(use, out var useId)) continue;
                            if (killedIdentifiers.Get(block.BlockIndex, useId)) continue;
                            upwardExposedIdentifiers.Set(block.BlockIndex, useId, true);
                            globalIdentifiers.Set(0, useId, true);
                        }
                    }
                    definesSet.Clear();
