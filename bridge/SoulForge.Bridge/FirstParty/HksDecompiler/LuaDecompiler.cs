    ...
    public class LuaDecompiler
    {
            ...
            LuaFile.Function luaMain)
        {
            ...
            void Visit(LuaFile.Function luaFunction, Function irFunction)
            {
                ...
                /* A register can be read before the first explicit
                 * definition (numeric-for loop variables are the important
                 * example: the VM materializes R(A+3) in FORLOOP).  Seed the
                 * IR with the native frame width so SSA analysis has a slot
                 * for every valid stack register, not only registers that the
                 * language-specific decoder happened to define. */
                irFunction.RegisterCount = Math.Max(
                    (int)luaFunction.NumParams,
                    (int)luaFunction.MaxStackSize);
