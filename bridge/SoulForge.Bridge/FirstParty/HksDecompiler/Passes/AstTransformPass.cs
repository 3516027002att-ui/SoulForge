...
public class AstTransformPass : IPass
{
    ...
    public bool RunOnFunction(DecompilationContext decompilationContext, FunctionContext functionContext, Function f)
    {
        ...
        void TransformNumericFor(BasicBlock node, BasicBlock loopInitializer)
        {
            ...
            Assignment incrementInstruction = node.Instructions
                .OfType<Assignment>()
                .LastOrDefault(instruction => instruction.Right is BinOp { Operation: BinOp.OperationType.OpAdd })
                ?? throw new Exception(
                    $"Numeric-for CFG shape is incomplete: expected an increment assignment before the loop jump, "
                    + $"got {string.Join(", ", node.Instructions.Select(instruction => instruction.GetType().Name))}.");
            Expression increment = (incrementInstruction.Right as BinOp)?.Right
                ?? throw new Exception(
                    $"Numeric-for increment expression is not a binary increment: "
                    + $"{incrementInstruction.Right?.GetType().Name ?? "null"}; "
                    + $"node=[{string.Join(" | ", node.Instructions.Select(instruction => instruction.ToString()))}].");
            ...
            if (node.EdgeFalse != node.Follow)
            {
                ...
                if (ifStatement.False.Last is Jump fj &&
                    (fj.Destination.BlockIndex != ifStatement.False.BlockIndex + 1
                     || fj.Destination == node.Follow))
                {
                    /* A structured if already falls through to Follow after
                     * the branch.  Keeping a CFG jump to that same follow
                     * makes FunctionPrinter emit an unresolved
                     * `goto basicblock_*` in otherwise valid Lua source. */
