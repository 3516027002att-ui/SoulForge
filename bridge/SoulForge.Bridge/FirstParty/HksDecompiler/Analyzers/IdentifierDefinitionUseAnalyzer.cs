...
namespace SoulForge.Bridge.FirstParty.HksDecompiler.Analyzers;
...
public class IdentifierDefinitionUseAnalyzer : IAnalyzer
{
    ...
    private ref IdentifierInfo GetIdentifierInfo(Identifier identifier)
    {
        ...
        {
            throw new Exception(
                $"SSA identifier is outside the register analysis table (register={identifier.RegNum}, subscript={identifier.RegSubscriptNum}).",
                ex);
        }
    }
    ...
    public void Run(DecompilationContext decompilationContext, FunctionContext functionContext, Function function)
    {
        ...
        for (var b = 0; b < function.BlockList.Count; b++)
        {
            ...
            for (var i = 0; i < block.Instructions.Count; i++)
            {
                ...
                {
                    /* A VM-provided register (notably the numeric-for value
                     * in a LuaP FORLOOP) may be read before the bytecode has
                     * an explicit SSA definition.  Such an identifier is
                     * intentionally left as a plain register by the SSA
                     * pass; it has no rename-info row to analyze. */
