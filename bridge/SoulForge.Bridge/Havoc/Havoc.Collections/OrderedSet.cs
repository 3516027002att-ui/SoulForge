...
public class OrderedSet<T> : IList<T>, ICollection<T>, IEnumerable<T>, IEnumerable, IReadOnlyList<T>, IReadOnlyCollection<T>
{
	...
	{
		if (!mIndexMap.TryGetValue(item, out var value))
		{
			return false;
		}
		mIndexMap.Remove(item);
		mList.RemoveAt(value);
		foreach (T m in mList)
		{
			int num = mIndexMap[m];
			if (num > value)
			{
