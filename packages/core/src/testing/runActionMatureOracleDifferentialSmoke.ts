...
interface DifferentialMetrics {
  ...
  worstQuaternion?: {
    ...
    oracle: [number, number, number, number];
  };
}
...
export async function runActionMatureOracleDifferentialSmoke(): Promise<void> {
  ...
  try {
    ...
    const animationContainerPath = join(gameRoot, 'chr', 'c0000_a000_lo.anibnd.dcx');
...
function compareClipToOracle(options: {
  ...
  let worstQuaternion: DifferentialMetrics['worstQuaternion'];
  ...
  for (const point of points) {
    ...
    for (let track = 0; track < sample.tracks.length; track++) {
      ...
      if (!worstQuaternion || rotationError > worstQuaternion.error) {
        worstQuaternion = {
          error: rotationError,
          track,
          label: point.label,
          hkxBone: hkxBone ?? -1,
          bridge: actual.rotation,
          oracle: expected.rotation
        };
      }
    }
  }
  ...
  if (worstQuaternion) metrics.worstQuaternion = worstQuaternion;
