...
const EVENT_REFERENCE_ENTRIES: readonly EventReferenceEntry[] = [
  ...
  { instruction: 'DisplayMinibossHealthBar', aliases: ['精英怪血条', '精英血条', '小boss血条', '头目血条', 'miniboss health bar', '显示精英怪血条', '精英怪全屏血条', 'miniboss血条'], description: '显示精英怪/小Boss/头目的专属全屏血条与架势条（包含红点生命槽）。' },
  ...
  { instruction: 'HandleMinibossDefeat', aliases: ['精英怪击杀', '精英怪死亡', '击杀精英怪', '头目击败', 'miniboss defeat', '击败精英怪', '精英怪结算', '普通击破'], description: '处理精英怪/头目击败结算流程（区别于大首领不死斩横幅）。' },
  ...
  { instruction: 'RequestCharacterAICommand', aliases: ['ai命令', '发送ai命令', '请求ai计划', 'ai command'], description: '向角色 AI 发送命令；必须配合角色的 Lua AI 计划，若 AI 脚本无对应计划则不生效。' },
  { instruction: 'RequestCharacterAIReplan', aliases: ['ai重规划', '重新规划ai', 'ai replan'], description: '请求角色 AI 重新规划。' },
  { instruction: 'SetCharacterImmortality', aliases: ['不死身', '无敌', '角色不死', '特殊忍杀', '处决下跪', '特殊处决', 'immortality', '取消特殊忍杀'], description: '设置角色的不死身状态（控制角色在特定血量或条件下是否受不死保护）。' },
  { instruction: 'SetCharacterAnimationState', aliases: ['不能动', '可以动', '开启动作', '关闭动画', '让单位行动', 'animation state'], description: '控制角色能否行动/动起来（与ChangeCharacterEnableState控制模型显示/隐藏完全不同）。' },
  { instruction: 'ChangeCharacterEnableState', aliases: ['显示人物', '隐藏人物', '启用人物', '禁用人物', 'enable state'], description: '控制角色是否启用与显示/加载（是让模型在世界中显示或消失，不等同于能否行动）。' },
  ...
  { instruction: 'IfInoutsideArea', aliases: ['进入区域', '区域内', '区域触发', 'inside area'], description: '判断玩家或实体是否在区域内/外（配合MSB区域事件编号AreaEntityID）。' },
  { instruction: 'IfEntityInoutsideRadiusOfEntity', aliases: ['距离触发', '距离范围', '半径判断', 'radius condition'], description: '判断两个实体间的距离范围半径（如实体A离实体B是否超过指定米数触发）。' },
  { instruction: 'WarpCharacterAndCopyFloor', aliases: ['拉到狼身边', '召唤到玩家旁', '传送到玩家', 'copy floor warp'], description: '召唤流核心拉人代码：把角色传送到目标（如玩家狼）身边并复制地面楼层信息。' },
  { instruction: 'WarpCharacterAndSetFloor', aliases: ['传送角色', '传送到点位', 'set floor warp'], description: '把角色传送到地图实体/区域点位并设定楼层。' },
  ...
  { instruction: 'SetObjectInteraction', aliases: ['禁用鬼佛', '物体交互', 'object interaction'], description: '设置物体交互类型和启用状态（如设置Grapple, Disabled可禁用钩绳或鬼佛交互）。' },
  { instruction: 'CreateObjectfollowingSFX', aliases: ['雾门', '创建雾门', '物体雾效', 'fog wall'], description: '为物体创建跟随特效（常用于雾门制作：配合地图编辑器物体ModelName o001402创建雾门）。' },
  { instruction: 'DeleteObjectfollowingSFX', aliases: ['关闭雾门', '删除物体特效', 'fog wall off'], description: '删除或关闭物体跟随特效（常用于关闭/消除雾门特效）。' },
  { instruction: 'DeactivateObject', aliases: ['关闭物体', '开启物体', '物体启用', 'object enable'], description: '控制地图物体的启用与物理碰撞阻挡状态（常配合雾门控制开启或关闭碰撞阻挡）。' },
  ...
  { instruction: 'SetAreaEnvmap', aliases: ['昼夜', '天气', '环境天气', '改变场景天气', 'environment map'], description: '改变场景昼夜与全局环境天气（参数0-10不同预设可切换不同白天夜晚光影）。' },
