...
全部 211 条已登记验证命令按层级列出。层级顺序即执行顺序（先快后慢，早失败早停）。
...
npm run test:agent-rag-postwrite
...
npm run bridge:verify:flver-skin-mapping
...
npm run test:action-corpus-coverage
npm run test:action-corpus-coverage:chr
npm run test:action-corpus-coverage:chr:full
...
| `EV-MISSION1-CONVERGENCE-20260825` | revalidates=`EV-MAIN-MERGE-CONVERGENCE-20260824`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; mission1 工程侧重验证：Action、Event、Map、Agent/RAG 变更后的公开回归与真实语料专项验证；冻结范围语义未变。 |
| `EV-MISSION1-CONVERGENCE-20260825-REGISTRY` | revalidates=`EV-MISSION1-CONVERGENCE-20260825`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; 工程侧重修复验证：登记新增 FLVER skin mapping native smoke；冻结范围语义未变。 |
...
|---|---|---|---|---|---|---|---|---|
| `SCOPE-SEKIRO-BUILD` | `H-RUNTIME` | `REL-SCOPE`（`passed`）、`REL-H`（`open`） | `supported` | `V0.5` | `native-verified` | 4 | 3 | Sekiro 1.6.x 文件/产品版本族与 build identity |
...
| `SCOPE-BEHAVIOR-ANIMATION` | `D-BEHAVIOR` | `REL-D`（`open`） | `deferred` | `V0.5` → 延期 `V0.6` | `unverified` | 0 | 5 | 真实 Sekiro corpus 中行为、动画与跨资源引用清单的深度语义解析（延期至 V0.6） |
| `SCOPE-BEHAVIOR-TAE` | `D-BEHAVIOR` | `REL-D`（`open`） | `deferred` | `V0.5` → 延期 `V0.6` | `candidate` | 0 | 5 | Sekiro 全部 TAE 布局、事件类型、时间轴、参数与动画引用的完整读写（延期至 V0.6） |
| `SCOPE-BEHAVIOR-ESD` | `D-BEHAVIOR` | `REL-D`（`open`） | `supported` | `V0.5` | `candidate` | 4 | 7 | Sekiro ESD 状态机、条件表达式、命令与跳转关系的只读解析（含转移边闭合图与悬空目标检出）；表达式与命令参数体的 RPN 字节码按不透明 (offset,length) 上报，写能力不在本范围内 |
| `SCOPE-BEHAVIOR-SCRIPT` | `D-BEHAVIOR` | `REL-D`（`open`） | `supported` | `V0.5` | `unverified` | 9 | 7 | 真实 Sekiro corpus 中 luabnd / action *.hks 脚本容器的条目枚举、只读字节码证据投影、经 Patch Engine 的整个内层文件替换/重读/回滚，以及**明文条目的源码级编辑**（容器内与 action 目录下少数以纯文本存放的脚本与符号表；字节码条目不在源码级编辑范围内） |
...
延期只读预览编辑器：`tae`、`esd`。两者已实现的读取与投影保留并在 UI 标记为 V0.6 预览，`countedAsReleaseEditor=false`，写入在 `editorCapabilityContract.releaseWriteEnabled`、`@soulforge/shared` 的 `DEFERRED_PREVIEW_EDITOR_KINDS` 与主进程 `EDITOR_DEFERRED_TO_V06_READONLY` 三层失败关闭。
