...
全部 192 条已登记验证命令按层级列出。层级顺序即执行顺序（先快后慢，早失败早停）。
...
另有 17 条 script 显式排除在验证调度之外（写入命令、外部工具或入口自身）：
