...
| `REL-D` | 行为动画范围 | `W-REL-D-GAMELOAD-01` | `open` | `in-scope` | `EV-BEHAVIOR-MAP-01-20260803`、`EV-SCRIPT-READONLY-01-20260803`、`EV-ESD-SUPPORTED-TRANSITION-EDGES-20260808`、`EV-SCRIPT-PLAINTEXT-SOURCE-EDIT-RULING-20260808` | 本 Gate 的 4 个范围条目中 animation/TAE 两项已延期 V0.6，`SCOPE-BEHAVIOR-SCRIPT` 与 `SCOPE-BEHAVIOR-ESD` 留在 V0.5，因此 Gate 保持 open 而非 deferred；后继要求 script 容器只读证据视图与整内层文件替换的写/重读/回滚/游戏加载闭环；ESD 只要求只读解析闭环（转移边跟随、悬空目标检出、剩余 RPN 缺口如实登记），不含 writer。TAE 登记样本 native document 与延期预览面板保留只读；ESD 面板同样保留只读（present-esd-panel-as-v05-release-editor 仍是禁令），不得据此声称本版行为语义能力 |
