...
状态：`partial / runtime unverified`

SoulForge 不实现自己的 Mod loader 或注入器。Sekiro 首选正式集成 me3，并保留可替换的通用运行接口：
...
- `RuntimeSessionManager` 管理 active session、等待、终止、后台终态持久化和进程重启后的 orphaned 恢复；
- `app.db` 的 `runtime_adapter_settings` 保存可信 me3 配置；`workspace.db` 的 `runtime_launch_sessions` 保存会话、日志、诊断和 operation 关联；
- 数据库 authority 运行在 Electron utility process，app-only 配置与 workspace session 均支持 utility 强制重启后恢复；
- 独立 `runtimeIpc.ts` 复用 main sender/document 校验，原生文件选择器持有真实路径，preload 只暴露脱敏 DTO；
- 工作区切换前终止并持久化旧活动会话，再切换 workspace database authority；
...
me3 是可替换的运行适配器，不是工作区、Patch Engine 或语义模型的核心依赖。当前只有 fake process、SQLite 和 IPC contract 证据，因此不得提升为 `native-verified` 或“游戏验证通过”。

### 发行状态：`partial / unverified`

已有：

- Windows CI 配置与按 smoke 拆分的诊断门；
- release content 扫描；
...
`skipped`、fake process、退出码 0 和 `unverified-no-local-sekiro-runtime` 都不能算真实游戏验证通过。
...
- 已实现：document instance 与稳定节点 anchor；受限 patch DSL tokenizer/parser/AST/source span；EMEDF schema fingerprint、revision、document instance 与 anchor 绑定；严格 scalar type/range 校验；`set id`、`set rest`、typed `set arg` 的确定性 mutation plan；unknown instruction 只读；patch template render -> parse -> bind -> empty plan。
- 已验证：公开 Windows CI 覆盖 typecheck、core smokes、EMEVD DSL compiler、desktop security、SQLite、Bridge 和 build。
...
- 起始：`2002076`；并行吸收 EMEVD mainline `4d37861`。
- 路线：A / H-me3 / H-发行
- 状态变化：H-me3 `not-started -> unverified foundation -> partial / runtime unverified`。
...
- 外部阻塞：当前执行环境没有合法 Sekiro 与 me3 运行环境。
