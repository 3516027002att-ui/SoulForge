...
| `W-REL-C-MULTILANG-01` | `active` | `partial` | — | `C-FMG` | 补 FMG 多语言与多 msgbnd 写验证（本机 corpus 仅 zhocn，需注册第二语言样本） | 本机 corpus 语言覆盖受限（mods/msg 仅 zhocn）；注册多语言 fixture 样本不触原版只读目录 | `bridge/SoulForge.Bridge/FmgNativeWriter.cs`、`packages/core/src/testing/runNativeFmgSmoke.ts` | `npm run bridge:verify:fmg`；`validation-unfrozen`：FMG 多语言写验证 | cap=`partial`；只覆盖实际注册的多语言布局 |
...
|---|---|---|---|---|---|
| `W-REL-C-MULTILANG-01` | `claim-w-rel-c-multilang-01-20260803` | fable-v05-driver | 2026-08-03T18:30:46.248Z | 2026-08-03T18:30:46.248Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
...
| `REL-C` | 核心语义 mutation 矩阵 | `W-REL-C-MULTILANG-01` | `open` | `in-scope` | `EV-EMEVD-FMG-PARAM-03-20260803`、`EV-FMG-MULTILANG-WRITE-20260804` | EMEVD DSL plan 的 production Bridge/PatchIR transaction 接线已完成；继续完整 EMEDF schema/control-flow、DSL 全局指令级 mutation、UI submit 接线；并行继续 FMG 全语言、全部 ParamType、MSB 实体和回滚 |
...
| `REL-G` | 双协议 AI | `W-AI-CONFORMANCE-03` | `passed` | `in-scope` | `EV-AI-CONFORMANCE-03-20260803`、`EV-AI-CODEX-KERNEL-20260804` | 错误/取消/超时/限额 10 case 已完成；继续真实工作区多步 typed mutation 矩阵 |
