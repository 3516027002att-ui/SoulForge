...
| C EMEVD | `partial` | `EV-C-EMEVD-DSL-20260724`、`EV-PRIVATE-20260724`、`EV-EMEVD-PATCHIR-20260731` | **DSL plan → Bridge batch staging → file_replace PatchIR → WorkspaceTransaction 的 production 接线已完成**（四视图 submit 入口，合成 3 case + 真实 common.emevd 事件级 mutation 通过，重读/回滚/failure 全链验证）；修复 writer 组合 mutation 的 id 重命名验证与既有重复事件 id 容忍；仍缺完整 EMEDF/control-flow、DSL 全局指令级 mutation 与全 corpus |
...
| `W-EMEVD-FULL-01` | `ready` | `partial` | — | `C-EMEVD` | 继续完整 Sekiro EMEDF schema 与类型覆盖、DSL control-flow validation、DSL 全局指令级 typed mutation、四视图 DSL 提交 UI 接线（desktop IPC）与全 corpus mutation 矩阵 | `W-EMEVD-PATCHIR-02` 已完成 production 接线；未知指令、opaque 尾部和 layer 变体不得被重编码；真实文档既有重复事件 id 容忍但不修改 | `packages/core/src/emevd/emedfSchema.ts`、`packages/core/src/emevd/dslCompiler.ts`、`packages/core/src/editing/emevdFourViewController.ts`、`apps/desktop/src/renderer/src/editors` | `npm run test:emevd-dsl-compiler`、`npm run test:emedf-schema`、`npm run test:emevd-plan-production`；`validation-unfrozen`：完整 EMEDF schema 覆盖与 control-flow smoke | cap=`partial`；只提升实际完成并验证的 schema/类型/接线 |
...
| C EMEVD | `bridge:verify:emevd`、`test:emevd-dsl-compiler`、`test:emedf-schema`、`test:emevd-four-view`、`test:emevd-ipc-contract`、`test:emevd-plan-commit`、`test:emevd-plan-production` | 已覆盖 header/event/instruction/args/mutation、稳定 anchor、DSL parse/typecheck/plan、UI 协议，以及 DSL plan → Bridge batch → PatchIR transaction 的 production 接线（stage/commit/re-read/rollback，合成 + 真实 common.emevd 事件级 mutation） | layer 变体、完整 EMEDF/control-flow、DSL 全局指令级 mutation、游戏加载 |
