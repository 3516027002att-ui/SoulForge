...
全部 159 条已登记验证命令按层级列出。层级顺序即执行顺序（先快后慢，早失败早停）。
  ...
  "scopeItems": [
    ...
    },
    {
      ...
      "nonClaims": [
        ...
        "**待用户裁定（不阻塞已交付范围）**：Shift-JIS(CP932)编码器缺失。三个 action/*nameid.txt 是 Shift-JIS，而 Node 只有解码器没有编码器（TextEncoder 固定输出 UTF-8，仓库无 iconv 依赖）。当前实现只允许结果为纯 ASCII 的编辑，含非 ASCII 一律以 PLAINTEXT_SHIFT_JIS_ENCODE_UNSUPPORTED 拒绝。补一张 CP932 编码表即可放开写入日文，但那是新增能力，需裁定后另立证据，不作为本次源码级写的顺带扩展。",
