...
| `W-EMEVD-DSL-01` | `completed` | `fixture-confirmed` | — | `C-EMEVD` | 已建立 DSL lexer/parser/AST、规范 render、EMEDF typecheck 与有序 typed mutation proposal；本切片验收边界已完成 | 复用 `emevd-editor-ir`；未知指令和不可无损重编码 payload 失败关闭/保持 opaque | `packages/shared/src/emevd-editor-ir.ts`、`packages/core/src/emevd/emevdDsl.ts`、`packages/core/src/emevd/emedfSchema.ts`、`packages/core/src/editing/emevdFourViewController.ts` | `npm run test:emevd-dsl`、`npm run test:emedf-schema`、`npm run test:emevd-four-view` | cap=`fixture-confirmed`；Bridge/PatchIR 与完整控制流进入后继切片 |
...
| C EMEVD | `bridge:verify:emevd`、`test:emevd-dsl`、`test:emedf-schema`、`test:emevd-four-view`、`test:emevd-ipc-contract` | 已覆盖 header/event/instruction/args/mutation、DSL parse/typecheck/proposal 和 UI 协议 | layer 变体、完整 EMEDF/control-flow、DSL proposal 的 Bridge/PatchIR 接线、游戏加载 |
...
| `EV-C-EMEVD-DSL-20260724` | `unsealed-record` | EMEVD DSL lexer/parser/AST、EMEDF typecheck 与 typed proposal | `2002076` + 当前工作树 | `npm run test:emevd-dsl`、`npm run test:emedf-schema`、`npm run test:emevd-four-view`、`npm run bridge:build`、本机环境注入后 `npm run bridge:verify:emevd` 均 exit 0 | fixture EMEDF；规范 DSL no-op 与 typed arg/rest proposal；未知/尾随 payload 保持；真实 `common.emevd` 1,730/33,266 的既有 mutation 回归及 uint32 负向 | 保留 DSL `fixture-confirmed` 观察；因基线未封存，不支持新的 authority 或 Gate 终态；proposal 未写二进制、未接 Bridge/PatchIR，完整 EMEDF/control-flow/layer/游戏加载仍未验证 |
...
- 起始：`2002076`
- 结束：未提交工作树；HEAD=`2002076` + `{packages/shared/src/emevd-editor-ir.ts, packages/core/src/emevd/emevdDsl.ts, packages/core/src/emevd/emedfSchema.ts, packages/core/src/editing/emevdFourViewController.ts, packages/core/src/testing/runEmevdDslSmoke.ts, packages/core/src/testing/runEmedfSchemaSmoke.ts, packages/core/src/testing/runEmevdFourViewSmoke.ts, apps/desktop/src/renderer/src/editors/EmevdFourViewPanel.tsx, bridge/SoulForge.Bridge/EmevdNativeWriter.cs, packages/core/src/testing/runNativeEmevdSmoke.ts, package.json, packages/core/package.json, docs/V0_5_IMPLEMENTATION_HANDOFF.md}`；提交后补 SHA
...
- 已验证：`npm run typecheck`、`npm run test:emevd-dsl`、`npm run test:emedf-schema`、`npm run test:emevd-four-view` exit 0；首次真实 `bridge:verify:emevd` 因运行旧 Bridge 二进制而命中负向断言 exit 1，执行 `npm run bridge:build`（0 warnings / 0 errors）后原样复跑 exit 0，1,730 events / 33,266 instructions、等长/变长 args、事件 add/delete GC 与负 `restBehavior` 拒绝均通过。
