...
全部 180 条已登记验证命令按层级列出。层级顺序即执行顺序（先快后慢，早失败早停）。
...
```json
{
  ...
  "scopeItems": [
    ...
    },
    {
      ...
      "subjectKind": "behavior-animation",
      "scope": "真实 Sekiro corpus 中 luabnd / action *.hks 脚本容器的条目枚举、只读字节码证据投影、经 Patch Engine 的整个内层文件替换/重读/回滚，以及**明文条目的源码级编辑**（容器内与 action 目录下少数以纯文本存放的脚本与符号表；字节码条目不在源码级编辑范围内）",
      "decisionStatus": "user-approved",
      "proposedSupport": "supported",
      "operations": [
        ...
        "source-level-edit-plaintext-script-entries",
        "write",
        "re-read",
        "rollback",
        "game-load"
      ...
      "nonClaims": [
        ...
        "三个 action/*nameid.txt 是 **Shift-JIS（CP932）** 而非 UTF-8，首行就是一句日文注释「#BOM付きUTF8で保存された場合に一行目が解析不能になる…」。按 UTF-8 读写会把所有日文替换成 U+FFFD 并改变字节长度，且不报任何错。Node 内置 TextDecoder 支持 shift_jis，但 TextEncoder 的 encoding 固定为 utf-8（传参数无效，实测 `new TextEncoder('shift_jis').encoding` 仍返回 utf-8），仓库也没有 iconv 依赖。**先前据此判定「回写方向没有编码器」并只允许纯 ASCII 编辑，那个结论下得太早**——编码表不必手写，可用解码器枚举反推（packages/core/src/script/plaintextScriptEntry.ts 的 encodeShiftJis）：解码器是权威，反推出的表与它天然一致。已实测三个真实文件的解码→编码往返逐字节一致（148045 / 91947 / 13682 字节）。CP932 覆盖不到的字符以 PLAINTEXT_SHIFT_JIS_CHAR_UNMAPPABLE 拒绝而不替换成 ?。不声明该表覆盖 CP932 全集——它与解码器一致，但未逐码位与外部权威表对照。",
        "`801000_battle.lua` 是**混合编码**：原版日文 + 后来加的 GBK 中文注释（偏移 107502 的字节 `0xc9 0xef` 无法按 Shift-JIS 解码，整段 `ÓÒÊÖµ¯·ÉïÚ` 是 GBK 的「右手弹飞镖」，显然是中文 mod 作者改过）。此前 detectPlaintextEncoding 把「不是 UTF-8」当成「就是 Shift-JIS」会误判它可处理，已新增 mixed-unknown 取值。**先前据此判定它「不可能无损、不接受源码级编辑」，那个结论也是错的**：整篇解码-编码往返确实会丢字节，但它有 2740 行纯 ASCII、仅 6 行含非 ASCII，走 `replace-ascii-bytes` 字节级动作（latin1 一一可逆映射 + 子串替换，不解码）后 52 个非 ASCII 字节完全不变。文件本身可以无损编辑，是实现强制了往返。可改数值与 ASCII 标识符，不可改其中的日文/中文注释（那需要知道该编成哪些字节，而该文件编码本身不一致）。",
        "端到端读写链路已在真实语料上验证（test:plaintext-script-write）：**文件侧 6/6**，全部 6 个目标条目各自走完「明文判定 → 源码级编辑 → Patch Engine 提交 → Bridge/磁盘重读一致 → 回滚逐字节还原」；**工具侧 1/1**，经生产 AI 工具 propose_plaintext_script_edit 的同链验证。两个计数分开报而不合并——先前报成「6/6」时分母是文件数而分子含工具那一条，实际是 5 个文件 + 1 个工具链，那种说法听起来像六个文件都过了。三条写路径各自验过：容器内条目走 container_child_replace、裸文件走 file_replace、混合编码条目走字节级 container_child_replace；任一条通过都推不出其余两条通过。",
        ...
        "**待用户裁定（不阻塞已交付范围）**：Codex 把 agent 权限拆成两个正交轴——`AskForApproval`（untrusted / on-request / granular / never，决定「何时问」）与 `SandboxMode`（read-only / workspace-write / danger-full-access，决定「能碰什么」），取自 openai/codex 仓库的协议 schema（Apache-2.0，只取名称做对照）。本项目只有一个 `permissionMode`（plan / normal / fullPermission）混合了两者语义。拆成两轴会改动 IPC 契约、权限阶梯与两处已封存断言（runV05FoundationSmoke:132/135、runAiToolPermissionSmoke 的 MODE_CEILINGS），属于范围变更，不作为审批层的顺带扩展。审批决定档位已按实测对齐（补齐 timed_out 与 abort）。",
        "（已解决，保留记录）Shift-JIS(CP932)编码器此前被判为缺失，故只允许纯 ASCII 编辑。实测那让三个 *nameid.txt 完全无法源码级编辑（首行即日文），随后用解码器枚举反推出编码表并验证往返逐字节一致，该限制已解除。保留此条是为了记下判断过程：「标准库没有编码器」不等于「无法编码」。",
        ...
        "裁定时 authorityAtRuling 为 unverified：容器条目枚举、只读证据投影、writer 与真实游戏加载证据仍待建立；本次放开源码级编辑不提升 authority。"
      ],
      "targetRelease": "V0.5",
      "deferredToRelease": null,
      "deferredTrack": null,
    ...
    {
      "scopeItemId": "SCOPE-MATBIN-53DE-DEFERRAL",
      "capabilityId": "E-ASSET",
      "gateIds": [
        "REL-SCOPE"
      ],
      ...
      "unsupportedOperations": [
        ...
        "claim-matbin-write-authority",
        "reuse-mtd-parser-for-matbin",
        "treat-suffix-as-confirmation"
      ],
      "authorityAtRuling": "unverified",
      "evidenceRefs": [],
      "registryRefs": [],
      "openRulings": [],
      "nonClaims": [
        "MATBIN 无确认 parser 时留 Files(53A 先例),不建 MatbinNativeDocument.cs;Sekiro 0 个 .matbin,格式无公开规范,建 parser 会违反「没有真实 parser 就不能声称格式已解析」硬约束;53D/53E 延期 V0.6,resume 需 Mod Engine mods/parts 真实语料与格式证据"
      ],
      "targetRelease": "V0.5",
      "deferredToRelease": "V0.6",
      "deferredTrack": "material-matbin",
      "resumeRequires": [
        "mod-engine-mods-parts-matbin-corpus",
        "matbin-format-spec-or-reverse-evidence",
        "confirmed-native-parser"
      ]
    }
  ]
}
