...
V0.5 的发布完成仍以当前 Gate、Evidence 与真实验证口径为准；这不等于冻结开发。过渡裁定已解除 `SCOPE-BEHAVIOR-ANIMATION`、`SCOPE-BEHAVIOR-TAE`、`SCOPE-BEHAVIOR-ESD` 与 `SCOPE-EDITORS` 的版本性 writer、unknown-structure reencode、release-editor 及其他开发限制，ACTION/TAE/ESD 与当前编辑器契约可以继续推进。资产聚合、MTD、collision、navigation、open-conversion 等仍为真实 deferred 条目；延期不等于完成，也不等于永久排除。裁定与延期口径以 `docs/governance/*.json` 为准。
...
│  ├─ EMEVD + EMEDF
│  └─ MSB → V0.6 目标线（开发开放，writer 仍受 native/Patch/回读门槛约束）
│
├─ D. 行为与动画主干
│  ├─ luabnd / action *.hks 脚本容器：条目枚举 + 只读字节码证据 + 整文件替换
│  ├─ TAE / animation events → 过渡期持续开发（motion identity / writer / editor）
│  ├─ ESD / state machines → 过渡期持续开发（transition / writer / editor）
│  ├─ animation / behavior references → ACTION identity 与跨资源验证持续开发
│  └─ 脚本反编译 / 重编译 / typed script mutation → 过渡期持续开发，当前未完成
│
├─ E. 场景与资产主干（V0.6 交付线，开发开放）
...
│
├─ F. 专业编辑器主干（当前契约七个；MSB/FLVER 保留独立 typed write 路径）
...
│  └─ tae / esd → 过渡期持续开发并进入当前 V0.5 能力投影；msb / flver 保留独立 typed write
│
├─ G. AI Agent 主干
...
这些路线存在依赖，但不构成强制的单线程实施顺序。接手者可以在不破坏前置 authority 和写入边界的前提下并行推进。标注“延期 V0.6”的条目仍不属于 V0.5 完成条件，也不阻塞 V0.5 完成；解除版本性开发限制也不等于对应 authority、Gate 或 release 已通过。
...
| `C-MSB` | MSB（V0.6 目标线） | native document、`A-RECOVERY` | 完整场景、运行验证、地图引用 | typed mutation 可继续开发；实际 authority 仍按 native/Patch/回读验证限定 |
| `D-BEHAVIOR` | 行为与动画 | Sekiro corpus 范围裁定、A/B 底座 | 招式链、状态机、跨资源行为编辑 | ACTION/TAE/ESD 可继续开发；脚本字节码的源码级编译缺口仍按技术事实处理 |
...
| `F-EDITORS` | 专业编辑器 | 对应 C/D semantic/native document | 可用工作台 | 当前 `scope.json` 的编辑器契约可继续开发；Hex 仍是只读证据视图 |
...
状态：`partial`（authority 不变）· 目标版本：`V0.6`（开发不因版本性门禁停止）

证据：`EV-C-MSB-SCENE-20260724`（unsealed-record）、`EV-C-MSB-7BD`（historical-record）、`EV-REL-SCOPE-20260731-TEXT-FIRST`（延期裁定）。

MSB 的目标版本仍登记为 V0.6，但过渡期不再以版本性规则阻断已接线的 typed mutation：`MsbScenePanel` 与 `resource.applyMsbMutation` 可以继续推进，写入仍必须经过 native document、Patch Engine、revision、重读与恢复门槛。已达到的 `partial` authority 不因目标版本变更；当前 Gate/发行状态也不会因能力契约打开而自动通过。
...
脚本容器、TAE、ESD 与 animation/behavior 引用均可在过渡期继续开发；是否纳入某个 release 完成条件，仍由当前 scope/Gate/Evidence 单独判定。
...
当前已建立登记样本上的 TAE native document parser（939 animations、23,711 events、81 event types）和 ESD native document parser（36 groups、295 states、315 conditions及 RPN bytecode），并有工作台投影；这些结果仍是注册样本上的 `candidate`，不证明完整事件语义、全部布局或 native authority。writer、unknown-structure reencode 与 editor 接线可以继续开发，但完成度必须由真实语料、重读/恢复和游戏边界验证决定。
...
状态：`partial / candidate`（authority 不变）· 资产聚合线仍为 `deferred → V0.6`（整条）
...
开放格式到 FLVER/TPF/MTD 的 native 导入与未验证的泛资产 writer 仍不得作为发布能力；已接线的窄范围 mutation 仍须按各自 native authority、Patch Engine、重读/恢复证据限定。资产聚合只读与导出线属于 `deferred`，V0.6 仍会交付，两者不可混用。
...
- 简体中文界面和术语扫描；
- 过渡期可继续开发：MSB、TAE、ESD、FLVER 工作台与各自 typed mutation；当前能力投影不等于真实文档功能验收或 release Gate 通过。Hex 仍只提供只读证据视图。
...
当前 V0.5 release inventory 登记 BND4、FMG、PARAM、EMEVD、script、TAE、ESD 七项；MSB/FLVER 保留既有独立 typed write 路径，但不因本次过渡裁定自动进入当前 inventory。各项仍必须共享 Bridge native document、revision、selection 与 typed mutation/整内层替换边界；没有对应 parser/schema/typecheck/native writer 的操作不得伪装成已完成。script 编辑器仍区分只读证据视图、明文条目编辑与整个内层文件替换，不把字节码伪装成源码编辑。Hex 只允许作为只读偏移与原始字节证据视图，不能形成 raw write 或绕过 native authority。

msb、tae、esd、flver 的版本性 deferred-preview 门禁已移除；当前 `EDITOR_CAPABILITY_CONTRACTS`、`@soulforge/shared` 与 release-editor smoke 会校验它们的能力投影和 typed mutation，而不是按版本拒绝开发或写链。TAE/ESD 进入当前 release inventory，MSB/FLVER 的既有 typed write 路径保持独立；实际资源写入仍不能绕过 native authority、Patch Engine、revision、重读、恢复和 Gate。

当前 inventory contract 已精确登记七项，其中 BND4 与 script 工作台已由前端 Agent 完成首版（`Bnd4WorkbenchPanel.tsx` 容器浏览/子项替换、`ScriptContainerPanel.tsx` 只读证据 + 用户字节整内层替换），ParamDefPanel 已接真实行数据与字段级编辑；MSB/FLVER 的独立 typed write 不属于本次 inventory 统计。inventory 登记不等于七个编辑器完成。规模访问缺口仍在：TAE/ESD/MSB/FLVER 仍为 bounded-window，具体功能验收与真实 authority 继续由对应切片负责。
...
V0.5 发行边界冻结为 Windows 10/11 x64 的 NSIS，仅限项目所有者控制的内部测试机器；代码签名不再是范围或验收项。该历史 V0.5 范围不包含 portable 或自动更新；V0.9.2 另行登记 GitHub Release 驱动的 Windows x64 更新链，不能把 V0.9.1 的无按钮版本改写成具备更新能力。不得在 notices/再分发权利未闭环时向外部测试者或公众分发。未签名安装包仍必须通过确定性 manifest/hash、内容扫描、干净机安装、升级、卸载和 runtime 完整性验证，但不声明发布者身份或 SmartScreen 信誉。
...
| F 专业编辑器 | `partial / acceptance candidate` | `EV-F-EDITORS-7BD` historical、`EV-HANDOFF-LIVENESS-20260725`、`EV-PUBLIC-CONTRACTS-20260725`、`EV-EMEVD-SCALE-20260731` | inventory 已精确冻结为 BND4/FMG/PARAM/EMEVD/script（文本优先裁定后 MSB/TAE/ESD/FLVER 降为延期预览）；Hex 已只读，FLVER 归资产查看器；**EMEVD 编辑器规模访问已提升为 `pagination`**（完整文档分页组装 + DSL 模板行数截断 + 事件列表分页，硬约束 17 合规，`currentScaleContractGaps` 不再含 EMEVD）；**script 只读证据面板、BND4 工作台与 ParamDefPanel 字段级编辑已接线（前端 Agent）**；**bnd4/fmg/param 规模访问已升级为 release-safe pagination**（主进程分页通道 `listContainerChildrenPage`/`readFmgPage`/`readParamPage` + renderer 面板按页导航）；**本机 mods 全量打开矩阵已落地**（`test:workspace-completeness`：243 文件逐个走生产读链，禁止抽样；2026-08-15 本机 231 opened / 0 语义失败 / 8 blocked / 4 history）；仍缺 Electron 逐文件 `selectFile`、GFX/BTL/SBLYT/HKX 语义 parser、写回和 REL-F |
...
| `W-FIRSTPARTY-SCHEMA-01` | `active` | `candidate` | — | `C-PARAM` / `C-EMEVD` | 解除生产运行时对 Smithbox/DarkScript3/Yapped 的依赖，随 SoulForge 发布自有版本化、内容寻址的 Sekiro 1.6.x PARAM/EMEVD schema；覆盖 160 个 PARAM definition、405 条 EMEVD 指令与 27 个 bank，覆盖缺口失败关闭 | 第三方 adapter 仅可用于开发与交叉验证，不得进入生产入口或发布包；C# Bridge 继续是 native 格式 authority；drawparam/gparam 仍延期至 V0.6 | `packages/core/src/schema/sekiro/firstPartySchema.ts`、`packages/core/src/param/paramMetadata.ts`、`packages/core/src/emevd/emedfRegistryResolver.ts`、`apps/desktop/src/main/ipc/param.ts`、`apps/desktop/src/main/ipc/event.ts`、`scripts/verify-first-party-schema-boundary.mjs` | `npm run test:first-party-schema-clean`；`npm run test:first-party-schema-boundary`；`npm run test:first-party-schema-package`；`npm run typecheck`；`npm run bridge:verify:synthetic`；`npm run test:first-party-schema-native-coverage`；manualChecks：清空 LOCALAPPDATA、SOULFORGE_EMEDF_PATH、SOULFORGE_YAPPED_SDT_ROOT 后仍可读取内置 PARAM 字段和 EMEVD 源码；真实 native corpus 可用时执行对应 PARAM/EMEVD smoke；未运行项保持 unverified/partial；notes：内置 schema loader 自动校验 origin、package/version、`contentDigest` 与覆盖计数；旧外部参数只返回 EMEVD_EXTERNAL_SCHEMA_FORBIDDEN，不扫描或读取外部路径。 | cap=`partial`；仅覆盖已内置并通过当前验证的 Sekiro 1.6.x schema；未覆盖或长度不匹配保持只读，Bridge 仍是 native authority |
...
| `W-BEHAVIOR-MAP-01` | `completed` | `candidate` | — | `D-BEHAVIOR` | 维持 script 容器（luabnd / action *.hks）的 magic/reference inventory 为只读证据视图；TAE/ESD native document parser 的既有登记能力保留为独立只读基础，不由本切片声明 ACTION identity 或 writer 完成｜已完成证据：script 容器 magic/reference inventory 已冻结（`test:script-container-evidence` 36 合成 case + 真实 leg：真实 luabnd aicommon.luabnd.dcx 301 条目/256 采样、扩展名 {lua:299,luagnl:1,luainfo:1}、magic 11/12 命中 \`x1bLuaP`、文本 goal_list.lua magicVerified=false 如实、条目名脱敏无绝对路径；真实字节 magic 修正：Sekiro luabnd 编译字节码为 \`x1bLuaP`（0x50）而非此前文档声称的 \x1bLuaQ，且容器内 .lua 为字节码+文本列表混合；probe:behavior-headers 增强（\x1bLuaP/HKX TAG0/TAE/ESD fsSL/LUAINFO 识别，SOULFORGE_SEKIRO_GAME_ROOT 缺失失败关闭）；`sanitizeEntryName` 导出供分页通道复用）。TAE/ESD native document parser 的既有登记样本仍由 Bridge 基础能力保留；ACTION motion identity、Binder membership 与预览调用由独立切片验证，不由本记录提前提升 authority | 合法 Sekiro corpus root/registry 已可用；不得套用其他游戏结论或把扩展名计数当 parser；探针输出不提交、不提升 authority；不得把字节码反汇编呈现为可编辑源码 | `packages/core/src/testing/probeBehaviorHeaders.ts`、`bridge/SoulForge.Bridge/TaeNativeDocument.cs`、`bridge/SoulForge.Bridge/EsdNativeDocument.cs`、Bridge inspection | `npm run bridge:verify:tae`、`npm run bridge:verify:esd`；`validation-unfrozen`：script 容器 magic/reference inventory smoke | cap=`candidate`；仅覆盖注册样本的容器级 inventory 与字节码格式识别，不证明脚本语义、反编译、重编译或 writer |
| `W-ACTION-MOTION-IDENTITY-01` | `completed` | `candidate` | — | `D-BEHAVIOR` | 完成 ACTION/TAE 链：TAE `animId` → `motionAnimId` → 同 character family 的 ANIBND Binder entry 唯一定位 → Bridge clip/pose；补齐 shared/core projection、overlay shadow、NOT_FOUND/AMBIGUOUS 失败关闭、base remount/source revision invalidation 与定向回归，并为后续 writer/editor 扩展保留同一 identity 入口 | 用户已解除 `SCOPE-BEHAVIOR-ANIMATION`、`SCOPE-BEHAVIOR-TAE`、`SCOPE-BEHAVIOR-ESD` 与 `SCOPE-EDITORS` 的 V0.5/V0.6 过渡期版本限制；writer、unknown-structure reencode 与 release editor 可继续独立开发；现有 `bridge/SoulForge.Bridge/TaeNativeDocument.cs` 未提交改动必须保留 | `packages/shared/src/resourceSymbols.ts`、`packages/core/src/indexing/ingestBridgeResult.ts`、`packages/core/src/indexing/workspaceIndex.ts`、`packages/core/src/action/taeAnimationBridge.ts`、`apps/desktop/src/main/ipc/action.ts`、`apps/desktop/src/main/ipc/workspace.ts`、`bridge/SoulForge.Bridge/TaeNativeDocument.cs` | `npm run test:action-motion-identity`、`npm run bridge:verify:tae`、`npm run typecheck`、`npm test`、`npm run build` | cap=`partial`；只覆盖真实 TAE motion identity、同 character family 的 ANIBND membership、只读 clip/pose IPC 与缓存失效，不覆盖 HKX 全语义、writer 或游戏加载 |
...
| `W-REL-F-ACCEPT-01` | `completed` | `candidate` | — | `REL-F` | 已建立当前编辑器 inventory、authority/revision/typed mutation、只读 Hex、完整有界访问与提前 pass 失败关闭 harness；2026-08-30 过渡裁定后，TAE/ESD 进入当前 inventory，MSB/FLVER 保留独立 typed write 路径并继续由各自验证负责 | 不运行真实 Electron 真实文档功能验收；不以 synthetic/demo、FLVER 资产查看器或固定窗口冒充编辑器完成；不要求量化容量/延迟门槛；任何编辑器仍须以对应 native、Patch Engine、revision、重读/恢复和功能证据建立完成度 | `packages/core/src/editing/releaseEditorAcceptance.ts`、`packages/core/src/editing/editorCapabilityContract.ts`、`packages/shared/src/editor-protocol.ts`、`packages/core/src/testing/runReleaseEditorAcceptanceSmoke.ts` | `npm run test:release-editor-acceptance`、`npm run test:desktop-live-editor-contract` | cap=`candidate`；harness 不等于当前编辑器 inventory 的真实发布通过 |
| `W-REL-F-SCALE-02` | `completed` | `candidate` | — | `REL-F` | 关闭剩余编辑器的规模访问缺口：script 仍为 scaleAccess=none，需要有界访问｜已完成证据：script 编辑器规模访问已升级为 release-safe pagination（`listScriptContainerEntriesPage` 主进程分页通道 + ScriptContainerPanel 显式分页，currentScaleContractGaps=[]）；bnd4/fmg/param 规模访问已升级为 release-safe pagination（主进程分页通道 resource.listContainerChildrenPage/readFmgPage/readParamPage，FMG_PAGE_SIZE=100/PARAM_PAGE_SIZE=20，renderer 面板按页导航不再 eager 物化；`editorCapabilityContract` bnd4/fmg/param scaleAccess=pagination）；EMEVD 编辑器规模访问已从 eager 提升为 pagination（完整文档分页组装 `readFullEmevdDocumentViaBridge` + DSL 模板行数截断 `renderEmevdPatchDslBounded`（事件块边界截断 + 注释标记，patch no-op 语义安全）+ 事件列表分页每页 200 + `loadFullDslTemplate` 显式完整加载；`currentScaleContractGaps` 不再含 EMEVD）；当前 V0.5 inventory 精确登记 BND4/FMG/PARAM/EMEVD/script/TAE/ESD 七项，MSB/FLVER 独立 typed write 路径不纳入 inventory；前端工作台已补（ScriptContainerPanel.tsx script 只读证据 + 用户字节整内层替换；Bnd4WorkbenchPanel.tsx BND4 容器工作台；ParamDefPanel 接真实行数据 + 字段级编辑）；真实文档完整有界访问已通过（`test:editor-bounded-access` 真实腿：script luabnd-primary 301/301 全量枚举 entriesComplete=true、EMEVD 34 页/33,266 指令/1,730 事件全覆盖，页间不重叠/并集全覆盖/末页触尾，共享 `normalizePageWindow` 无私有副本；`test:desktop-ipc-contract` 51 断言真实执行观测：main 分页 channel 注册 + preload 分页方法接线 + 双向对账）；Electron 面板功能验收仍单独门控（opt-in，不在此声明） | Hex 保持只读证据视图；当前 candidate 只继承 acceptance harness 对缺口的分类；EMEVD pagination 只证明规模访问契约（分页组装/模板截断/列表分页），不证明完整语义编辑或真实验收；TAE/ESD/MSB/FLVER 的 bounded-window 与功能验收继续由对应切片推进，不因 inventory 统计自动提升 authority | `packages/core/src/editing/editorCapabilityContract.ts`、`packages/core/src/editing/emevdFullDocument.ts`、`packages/core/src/emevd/dslRenderer.ts`、`apps/desktop/src/main/ipc.ts` | `npm run test:release-editor-acceptance`、`npm run test:desktop-live-editor-contract`、`npm run test:emevd-full-document`、`npm run test:emevd-dsl-compiler`；`validation-unfrozen`：真实文档完整有界访问与 Electron functional smoke | cap=`partial`；只提升实际完成的编辑器功能，不以 inventory 或取消量化门槛代替真实验收 |
| `W-REL-F-ACCEPT-02` | `completed` | `candidate` | — | `REL-F` | 各编辑器结构化 UI/DSL 与完整有界访问收尾、Electron 真实文档功能验收（opt-in）与 BND4/script 工作台继续 | `W-REL-F-SCALE-02` 已完成（currentScaleContractGaps=[]）；Electron 真实文档功能验收为 opt-in 工程项，未跑不构成完成声明；TAE/ESD/MSB/FLVER 的版本性开发限制已解除，但 inventory、authority 与 release pass 仍以各自真实证据为准 | `packages/core/src/editing/editorCapabilityContract.ts`、`packages/core/src/editing/releaseEditorAcceptance.ts`、`apps/desktop/src/main/ipc.ts` | `npm run test:release-editor-acceptance`、`npm run test:desktop-live-editor-contract`、`npm run test:desktop-ipc-contract`；`validation-unfrozen`：Electron 真实文档功能验收 | cap=`candidate`；Electron 真实文档功能验收完成前不提升 `REL-F` |
...
|---|---|---|---|---|---|
| `W-FIRSTPARTY-SCHEMA-01` | `claim-w-firstparty-schema-01-20260916` | codex | 2026-09-16T10:42:53.783Z | 2026-09-16T10:42:53.783Z | 若本会话中断，核对 codex/firstparty-schema 工作树与未完成验证后再释放或继续 |
...
| 工作区前端完成度（全量读） | `packages/shared/src/workspace-format-family.ts` 按后缀定打开种类；`apps/desktop/src/renderer/src/workbench/documentLoadGates.ts` / `selectEditor.ts` 决定前端读哪条 IPC；Bridge `NativeLeafPayload.cs` 解一层 DCX/BND4 后交给 TPF/ESD/FMG/MSB 叶子 parser（与 FXR 同构，TS 不维护第二套 unpacker） | `packages/core/src/testing/runWorkspaceCompletenessMatrixSmoke.ts`（`npm run test:workspace-completeness`）；前端路由/加载门由 `npm run test:renderer-unit` 覆盖 |
...
npm run test:first-party-schema-boundary
npm run test:first-party-schema-package
...
npm run test:agent-native-proof-gate
...
npm run test:editor-catalog
npm run test:editor-catalog-schema
npm run test:editor-document-store
npm run test:editor-layout
...
npm run test:reference-optimization-performance
npm run test:reference-query
...
npm run bridge:verify:esd-writer
npm run bridge:verify:fxr
...
npm run test:container-param-writeback
...
npm run test:reference-optimization-native
...
| F 专业编辑器 | `test:editor-document-store`、`test:hex-scene`、`test:desktop-live-editor-contract`、`test:release-editor-acceptance`、`test:ui-localization`、`test:emevd-full-document`、`test:emevd-dsl-compiler`、`test:workspace-completeness` | document/revision/IPC/静态本地化契约；冻结五项 inventory（bnd4/fmg/param/emevd/script）、只读 Hex、FLVER 排除、无量化门槛的完整有界访问 schema 与失败关闭；EMEVD 规模访问 `pagination`（分页组装、DSL 模板行数截断 + 完整模板按需加载、事件列表分页，`currentScaleContractGaps` 不含 EMEVD）；script 只读证据面板、BND4 工作台与 ParamDefPanel 字段级编辑的 IPC/契约接线；本机 mods **每个索引文件**走生产读链打开（或诚实 blocked/history），禁止抽样 | bnd4/script `scaleAccess=none`、fmg/param `bounded-window` 完整有界访问、TAE/ESD 写链、八个真实语义编辑器、真实 Electron 逐文件点击、GFX/BTL/SBLYT/HKX 语义 parser、写回、REL-F |
...
`test:workspace-completeness` 扫本机 mods **每一个**索引文件（禁止按族抽样）。加载门按路径/后缀，不按顶层目录名。无语料结构化 skip。2026-08-15 本机 `$SOULFORGE_SEKIRO_GAME_ROOT/mods`：243 scanned、231 opened、0 语义失败、8 blocked（4 `behbnd` HKX 裁定 + 2 `.gfx` + 1 `.btl.dcx` + 1 `sblytbnd` 无 parser）、4 history（`.bak`）。墙钟约 45 秒。首次跑曾红 66 个，根因是把 DCX 当叶子读（魔数 `0x00584344` = `DCX\0`）；`read-tpf` / `read-esd` / `read-fmg` / `read-msb` 补一层容器解包后清零。本命令不证明 Electron 逐文件点击、写回或 native-verified。
...
| `EV-CMDSET-STAGINGCODE-MTD-20260808` | Bridge 命令集三方对账门禁 + 写入成功码单一声明点：read-mtd-document 按既有 V0.6 延期裁定从 dispatch 撤下，广告面补齐 5 个漏报命令（`scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved——范围语义未变，MTD/FLVER/TPF 仍为 V0.6 延期，本轮为工程侧重验证） |
| `EV-GATE-BLINDSPOT-ESD-TPF-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-CMDSET-STAGINGCODE-MTD-20260808`；工程重封存：探针残留门禁扩到仓库根（旧范围只扫 scripts/，换目录即失明）；两条 grep 式契约 smoke 的恒假判据与注释散文判据换成真判据；TPF smoke 的 `ddsValid` 名不符实、`fourCC` 拿到手不断言；ESD 声明量与实解析量双双导出却从不比较，补覆盖率判据后真实语料 12/12 降为 partial。范围未变，authority 未提升 |
| `EV-BC7-PAGINATION-ESD-SENTINEL-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-CMDSET-STAGINGCODE-MTD-20260808`；revalidates=`EV-GATE-BLINDSPOT-ESD-TPF-20260808`；revalidates=`EV-RECOVERY-AIPERM-COVERAGE-20260807`；工程重封存：DDS 补 BC7 解码（8 mode 全实现）与 111 断言常驻门禁；FLVER 补 endian/version 前置拒绝；ESD 覆盖率判据改按物理记录数对齐 0x30 并新增哨兵模型守卫；TPF 补重建覆盖面测量；TAE 类文档措辞诚实化；`readRawRange` 接线；列表静默截断与页大小三处双写收敛。范围未变，authority 未提升 |
| `EV-BC3-FLVERGAP-AIWIRE-NATIVEROOT-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-BC7-PAGINATION-ESD-SENTINEL-20260808`；revalidates=`EV-RELB-SCHEMAVER-COMPAT-20260808`；工程重封存：BC3 颜色块误用 BC1 punchthrough 已修；FLVER 未解析缺口接入 authority 降级（`native-verified` 到 partial，是下调不是提升）；AI agent 六通道接线；native 全量跑暴露的四条 FAIL 全部修复（三条同属语料根语义混用漏改点，涉及 `REL-B` 语料主题域）。范围未变，authority 未提升 |
| `EV-T14-WIRING-COMPLETE-FLVER-PARTIAL-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-BC3-FLVERGAP-AIWIRE-NATIVEROOT-20260808`；工程重封存 + 一处用户裁定的 authority 下调：T1-4 十五条待接线通道全部接线（preload 暴露面 57 条现已全部被 renderer 引用，`ruledNotYetWired` 归零）；按用户裁定把 `SCOPE-ASSET-FLVER` 的 `authorityAtRuling` 从 `native-verified` 下调为 partial。范围未变，authority 未提升（FLVER 那处是下调） |
| `EV-PNGCOLORSPACE-DDSTRUNC-TFMGATE-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-T14-WIRING-COMPLETE-FLVER-PARTIAL-20260808`；工程重封存：导出 PNG 补 DDS 声明的色彩空间（真实语料 36/52 纹理此前丢失 `sRGB` 声明）；截断 DDS 从静默导出纯黑图改为失败关闭；陈旧 TFM 构建产物门禁接线。范围未变，authority 未提升 |
| `EV-ESDGAP-XMACHINE-PARAMKRAK-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-PNGCOLORSPACE-DDSTRUNC-TFMGATE-20260808`；工程重封存：ESD 未解析结构（状态转移边 + commandID）改为 authority 可见；跨机复现判定逻辑补负向 fixture；native 层 41 条全量跑完（0 failed）；PARAM「31% 解析失败」根因经实测更正。范围未变，authority 未提升 |
| `EV-ESD-SUPPORTED-TRANSITION-EDGES-20260808` | `SCOPE-BEHAVIOR-ESD` 由用户裁定（2026-08-08）从 V0.6 延期改回 V0.5 supported，范围限只读解析；实现 condition 的 +0x00 `targetStateOffset` 与 command call 的 +0x04 `commandID` 两处此前从未读取的字段；冻结门禁收紧为按条目解冻。`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-SCRIPT-PLAINTEXT-SOURCE-EDIT-RULING-20260808` | `SCOPE-BEHAVIOR-SCRIPT` 按用户裁定（2026-08-08，同日第二次）新增 operation source-level-edit-plaintext-script-entries；unfreezeRuling.`scopeItemIds` 扩为 [`SCOPE-BEHAVIOR-ESD`, `SCOPE-BEHAVIOR-SCRIPT`]。本条只登记范围裁定与实测边界，不含任何实现。`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-AGENT-TOOL-SCHEMA-EXPOSURE-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-T14-WIRING-COMPLETE-FLVER-PARTIAL-20260808`；工程重封存：生产 AI 工具向模型暴露真实字段名与类型。`agentToolBridge` 此前投影不带 properties 的空壳 parametersJsonSchema，模型只能猜字段名；现改为从 `validateToolInput` 已强制的同一份 ToolInputShape 投影，并收紧 validate_patch / build_patch_graph 的整 proposal 声明、为四处枚举加 enum: 语法与运行期强制。范围未变，authority 未提升 |
| `EV-AGENTIC-APPROVAL-WIRING-PLAINTEXT-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-AGENT-TOOL-SCHEMA-EXPOSURE-20260808`；工程重封存：三件事一并推进。(1) agent 审批层——写类工具执行前必经用户检查点，loop 层四档决定与会话内记忆，IPC 双向通道，审批 UI 含三档危险度、改动预览与四档按钮；(2) 六项能力接线——`timeoutMs` / `maxSteps` / streaming / compaction / `retryPolicy` / `contextBroker` 从 IPC 到 loop 打通；(3) 明文脚本条目源码级写——按真实字节判定明文、Shift-JIS 编码边界显式拒绝、尾部对齐填充保留、写后重读比对，走既有 container_child_replace 不新建 writer。范围未变，authority 未提升 |
| `EV-PERMISSION-JUDGE-UNIFIED-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-AGENTIC-APPROVAL-WIRING-PLAINTEXT-20260808`；工程重封存：统一 plan 模式权限判据。`agentLoop` 此前用按名字硬编码的白名单与 `toolPermissions` 的 `maxPermissionForMode` 各表达一套 plan 语义；现改为等级阶梯唯一权威，白名单降级为 PLAN_MODE_EXTRA_DENY 显式收紧清单（只能更严、每条带实测理由）。纯重构，行为逐工具与统一前一致，范围未变，authority 未提升 |
| `EV-PENDING-RULINGS-RECORDED-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-PERMISSION-JUDGE-UNIFIED-20260808`；工程重封存：把两项待用户裁定与一处调研缺口从口头记录转为文件内可查——Shift-JIS(CP932) 编码器缺失写入 `SCOPE-BEHAVIOR-SCRIPT` 的 nonClaims，plan 上限是否收到 analyze 写入 `maxPermissionForMode` 头注，Codex 插件界面调研未返回结论写入 AgentApprovalPanel 头注并说明本组件形态的真实依据。纯注释与 nonClaim，无行为改动，范围未变，authority 未提升 |
| `EV-APPROVAL-UNIFIED-DIFF-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-PENDING-RULINGS-RECORDED-20260808`；工程重封存：审批卡片改用真正的 before/after unified diff。此前是单侧「将要写什么」预览，理由写的是「审批发生在执行之前、磁盘上没有改动可读」——该理由不成立：原文件可读、新内容在参数里、`createUnifiedDiff` 早已存在于 patch/`textDiff.ts`、PatchChange 一直有 diff 字段。范围未变，authority 未提升 |
| `EV-CODEX-DECISION-PARITY-20260808` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-APPROVAL-UNIFIED-DIFF-20260808`；工程重封存：按 Codex 实测形态补齐审批决定档位。此前两次派出的界面调研均未返回，本轮直接查官方文档与 openai/codex 协议 schema（Apache-2.0，只取名称与形态做对照，不复制源码），据此补 timed_out 与 abort 两档。范围未变，authority 未提升 |
| `EV-PLAINTEXT-RW-CHAIN-COMPLETE-20260809` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-CODEX-DECISION-PARITY-20260808`；工程重封存：打通六个明文条目的读写链路，文件与 AI 工具两侧。新增 `test:plaintext-script-write` 端到端 smoke（真实语料 6/6）、propose_plaintext_script_edit 工具、由解码器反推的 CP932 编码器、mixed-unknown 编码判定。范围未变，authority 未提升 |
| `EV-MIXED-ENCODING-BYTE-EDIT-20260809` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-PLAINTEXT-RW-CHAIN-COMPLETE-20260809`；工程重封存：混合编码条目改用字节级路径,六个目标文件全部打通。新增 replace-ascii-bytes 动作,文件侧 6/6 与工具侧 1/1 分开计数。范围未变,authority 未提升 |
| `EV-SMITHBOX-PARAM-FIELDS-EDITOR-LAYOUT-20260810` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-MIXED-ENCODING-BYTE-EDIT-20260809`；工程重封存：按用户指定接入 Smithbox SDT 2.2.4 元数据使 PARAM 字段名与数值可用（只读），并把证据投影收进折叠区、移到编辑器之后。范围未变，authority 未提升 |
| `EV-WORKBENCH-PARAM-TRUST-20260810` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-SMITHBOX-PARAM-FIELDS-EDITOR-LAYOUT-20260810`；revalidates=`EV-REBASE-REANCHOR-20260808`；工程重封存：按用户要求把 param 等工作页面从「重证据、不可编辑」改造为 Smithbox 式多栏工作台。（1）Hex 视图从抢占主视图改为收进默认收起的证据折叠区，previewKind==="hex" 是所有二进制格式的默认分支，此前打开任何资源首屏都是只读 Hex 证据；（2）PARAM 分页参数真正下传 Bridge，实测 C# 载荷门控按页算，全表读取必然超限导致每行 `dataBase64` 全为 null，字段解码/行复制/字段编辑因此都拿不到输入；（3）新增容器内 param 的解包读取通道（read-dcx-document 枚举 → extract-bnd4-child 解包到会话暂存区 → read-param-document），此前容器路径直接喂给 read-param-document 会报 PARAM 类型名偏移无效；（4）接通 paramMetadataTrustStore，字段写入由包校验+行宽核对+用户信任策略三层放行，取代原先硬写 origin=fixture 的恒只读；（5）MSB/ESD/TAE/TPF 改为分栏只读工作台并统一行选择可达性。范围未变，authority 未提升。 |
| `EV-CONTAINER-PARAM-WRITEBACK-20260810` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-WORKBENCH-PARAM-TRUST-20260810`；工程重封存：打通容器内 PARAM 字段写回的最后一段。上一条证据把「字段写回 parambnd 容器」列为未接通缺口（write-param 只能产出裸 param 暂存文件，没有路径塞回 BND4 再压 DCX），本次补齐：新增 resource.applyContainerParamFieldMutation，链路为 `applyParamFieldMutation` 编码 → write-param 出裸 param（经 stageBridgeOutput）→ write-bnd4 replace 塞回容器 → `applyNativeMutation` 的 Patch Engine commit port 落盘。授权门（origin 必须 imported/user-derived）与并发保护（`expectedContainerHash` + `expectedChildHash` 由后端给出、渲染器原样回传、C# 侧比对）两道前提不可绕过；解包缓存命中增加条目哈希校验，避免容器写回后拿过期哈希做并发比对。新增 `test:container-param-writeback` 门禁（synthetic tier，11 条断言，打在真实注册的 IPC handler 上）。范围未变，authority 未提升。 |
| `EV-WORKBENCH-EDITOR-SELECTION-RECENTPATHS-20260810` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-CONTAINER-PARAM-WRITEBACK-20260810`；工程重封存：按用户两轮反馈重构工作页面形态，并补「记住上次打开的目录」。（1）主视图从「15 个互不排斥的条件块叠出一叠卡片」改为 `selectEditor` 纯函数给出唯一编辑器，打开 parambnd 不再同时渲染 param 工作台与通用 BND4 容器工作台两张条目表；（2）诊断码与解析细节（DCX_PAYLOAD_BOUNDARY_CONFIRMED、「分页通道不可用」、「空文档（未选中可解析 EMEVD 或读取失败）」）全部移入新增的底部日志区，主编辑区只放数据；（3）param 行表改虚拟滚动（新增 @tanstack/react-virtual@3.14.9，MIT，已入许可证清单 44→46）并按对照结论加搜索结果 memo + 220ms debounce；（4）读取失败的 param 保留在列表并标记，点开显示结构化诊断 —— 不照抄参照工具「失败项从列表移除」的形态；（5）目录选择对话框记住上次位置（工作区与原版目录分开记），启动自动挂载上次工作区，凭据仍由主进程签发不接受入参。范围未变，authority 未提升。 |
| `EV-ROOT07-BRIDGE-ROOTS-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-WORKBENCH-EDITOR-SELECTION-RECENTPATHS-20260810`;ROOT-07 Bridge allowed-root 生命周期(front-`end.md` §13.2/§18.11)重验证后重封存,冻结范围语义未变 |
| `EV-PARAM-10A-PRIMARY-READ-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-ROOT07-BRIDGE-ROOTS-20260813`;PARAM-10A primary read(front-`end.md` §18.14)完成:两个 PARAM 读取通道拒绝 backup、失败路径非空结构化诊断、Mod DFLT 不依赖 base mount/staging,冻结范围语义未变 |
| `EV-PARAM-10BC-SMITHBOX-FOURCOLUMN-WRITEBACK-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-PARAM-10A-PRIMARY-READ-20260813`;PARAM-10B/10C(front-`end.md` §18.14)完成:Smithbox 四栏工作台与字段写回链验证,冻结范围语义未变 |
| `EV-GPARAM-11A-NATIVE-READ-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-PARAM-10BC-SMITHBOX-FOURCOLUMN-WRITEBACK-20260813`;GPARAM-11A(front-`end.md` §18.15)完成:从零逆向 Sekiro GPARAM v5 格式的 native 只读解析与 bank catalog,冻结范围语义未变 |
| `EV-GPARAM-11B-WORKBENCH-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-GPARAM-11A-NATIVE-READ-20260813`;GPARAM-11B(front-`end.md` §18.15)完成:Banks/Groups/Fields/Values/Tools 四栏工作台,冻结范围语义未变 |
| `EV-GPARAM-11C-WRITEBACK-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-GPARAM-11B-WORKBENCH-20260813`;GPARAM-11C(front-`end.md` §18.15)完成:typed 字段写回 loose/DCX/container 三 profile,冻结范围语义未变 |
| `EV-MODEL51-TEXTURE52-20260813` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-GPARAM-11C-WRITEBACK-20260813`;MODEL-51A/B/C(FLVER native 读写 + flver-editor 分页 DTO + 三栏 Model 工作台)与 TEXTURE-52A/B/C(TPF native 读 + typed replace 写 + 四栏 Texture 工作台)全链路完成并 `native-verified`,连带跨卡域工作台重构(EVENT-30A/B/C、TEXT-20A/B/C、SCRIPT-41、CONTAINER-40)首次完整实现无回归,冻结范围语义未变 |
| `EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-MODEL51-TEXTURE52-20260813`;MATERIAL-53A/BEHAVIOR-55A/ANIMATION-56A(front-`end.md` §18.19)三张 A 卡 read DTO 与 C# envelope 三页投影:material-`editor.ts`(MtdDocument/三页投影/窄守卫)、behavior-`editor.ts`(EsdDocument declared-parsed 对照+`transitionGraph` 四态)、animation-`editor.ts`(TaeDocument+TAE_INVALID_TIME_RANGE);C# MtdNativeDocument 升级三页 wire 形状与 authority 判据、EsdNativeDocument 补 transitionGraph/commandCalls/coverage、TaeNativeDocument 补诊断;read-mtd-document 命令三处入口恢复;bridge:verify:mtd 注册 tiers synthetic;esd/tae smoke 缺语料诚实跳过;daemon Release publish 重建含 read-mtd-document |
| `EV-VFX54-FXR-READ-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`;VFX-54A(front-`end.md` §18.19)FxrNativeDocument.cs 只读解析(文件头 0x90/14 section/offset 边界校验)+vfx-`editor.ts` 三页投影 DTO(FxrDocument/effect/nodes/fields/unparsedGaps/roundTrip/authority)+窄守卫;read-fxr-document 五处入口(BridgeCommandService dispatch/BridgeDaemonHost advertise/bridge-protocol TS union/runBridge/ipc/preload);bridge:verify:fxr 合成 FXR3 双 leg(真实经过 C# 解析);advertisement gate 38 三方一致;daemon Release publish 重建 |
| `EV-MATBIN-53DE-DEFERRED-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-VFX54-FXR-READ-20260814`;MATBIN 53D/53E 延期裁定:`scope.json` 加 `SCOPE-MATBIN-53DE-DEFERRAL`(user-approved,V0.6 deferred);Sekiro 原生与测试 Mod 语料 0 个 .matbin,Mod Engine 2 专用序列化格式非 FromSoftware 原生,53A 卡先例「MATBIN 无确认 parser 时留 Files」,unlistedPolicy=unsupported;不建 MatbinNativeDocument.cs parser(硬约束「没有真实 parser 就不能声称格式已解析」);editor-catalog matbin-suffix-candidate 已留 Files(bridge-confirmation-required);resumeRequires=Mod Engine mods/parts 真实语料+格式证据+确认 parser |
| `EV-MATERIAL53C-WRITEBACK-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`;MATERIAL-53C MTD 材质属性写回:MtdNativeWriter.cs 字节外科替换 writer,typed property set(paramId+newValue) 经 `applyNativeMutation` 走 Patch Engine;unknown 属性/未解析 gap 无损保留;未知结构 fail-closed 拒写(MTD_WRITE_BLOCKED_UNKNOWN_STRUCTURE);ipc 层 `commitMtdPropertySet` 带 `isParamBackupPath` 拒绝;preload/editing/`materialBridgeCommit` 全链路接通 |
| `EV-MAP50C-MSB-WRITEBACK-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;MAP-50C MSB 写回:MsbNativeWriter delete mutation(delete_part/delete_region/delete_event)按唯一名解析,param 偏移表软删除(offsetCount-1、表尾前移清零、条目字节留原址),其余实体逐字节无损;reopen-failure before-image 恢复;未注册/不存在目标 fail-closed;ipc `applyMsbMutation` 联合类型加 delete 三种 |
| `EV-VFX54B-WORKBENCH-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-VFX54-FXR-READ-20260814`;VFX-54B Smithbox 式 VFX 工作台:Effect/Particle list \| 真实预览(诚实空态,无假 viewport) \| Inspector 三栏只读;known/unknown node 状态明确(未知类型权威信号来自 C# `unparsedGaps`,renderer 不硬编码闭集);partial 缺口可见;App.tsx `readContract` 注册 readFxrDocument→vfx + .fxr/.fxr.dcx 后缀补正 |
| `EV-MATERIAL53B-BEHAVIOR55B-ANIMATION56B-WORKBENCH-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`;MATERIAL-53B/BEHAVIOR-55B/ANIMATION-56B 三 Smithbox 工作台(MaterialWorkbenchPanel/EsdWorkbenchPanel/TaeWorkbenchPanel):各自按 §2.5 三栏只读;unknown property/未知结构可见但不可编辑不丢弃;无假 Tools 栏;App.tsx 注册 + .mtd/.esd/.tae 后缀补正;e2e 文件数断言级联同步 |
| `EV-AGENT60A-DOCK-SHELL-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;AGENT-60A dock shell 与欢迎区:48px header/minmax conversation/底部 composer grid;340/440/620px、4px resizer、16px keyboard resize、Ctrl+J、workspace persistence、小空间 overlay;只使用 §12.4 固定欢迎文案;无旧 task panel/tool count/session count |
| `EV-BEHAVIOR55C-ESD-WRITEBACK-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`;BEHAVIOR-55C ESD 状态转移写回:EsdNativeWriter 三条 mutation(set-transition-target 字节级外科替换/insert-transition 表内新增并重定位 condition-offset 表/set-command-arg 恒 fail-closed);哨兵槽同步;真实语料暴露的 insert 表校验 bug 已修;unknown 保留(`unparsedGaps` 逐项一致,authority 保持 partial);ipc `commitEsdTransition` + 四方命令同步(广告 40 三方一致) |
| `EV-AGENT60B-COMPOSER-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-AGENT60A-DOCK-SHELL-20260814`;AGENT-60B 三层 Composer:participant 层(Ask/Plan/Edit 模式+权限回显) + prompt+chips 层(自动增高 40vh cap) + toolbar 层(固定 @\|#\|附件\|模型\|Plan\|发送/停止);IME composing 禁 Enter 发送(`isComposing` 双保险);输入状态机抽纯函数;附件/# 未接通诚实 disabled;模型按钮打开既有设置抽屉 |
| `EV-ANIMATION56C-TAE-WRITEBACK-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`;ANIMATION-56C TAE 事件写回:TaeNativeWriter 两条 mutation(update-event-times 字节级外科替换 float32 / insert-event 参数体逐字节拷贝自模板事件,span 由同动画事件数据块连续不变量确定);多 mutation 批改为对源基线一次性整体校验;插入事件不自动登记事件组(组语义未解码,如实标注);真实 a00.tae 890/890 非空动画成立;ipc `commitTaeEvent` + 四方命令同步(广告 41 三方一致) |
| `EV-AGENT60C-SELECTION-STREAM-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-AGENT60B-COMPOSER-20260814`;AGENT-60C selection context/资源引用/流式任务:agent-`ui.ts` 纯逻辑(snapshot 深度冻结/路径白名单/sender token 拒绝/严格 seq/bounded pages RETAIN_LIMIT=200/resume 尾部回放);ipc `sendAgentEvent` 按 session 单调 seq + agent.resourceReference.create 白名单通道(未加 BACKUP_READ_FORBIDDEN,ParamWorkbench 保持 7);selectedFilePath/`contextLabel` 降为过渡投影,EditorSelectionContext 为权威输入 |
| `EV-AGENT60D-TOOL-APPROVAL-DRAWER-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-AGENT60C-SELECTION-STREAM-20260814`;AGENT-60D tool activity/审批/二级抽屉:AgentConversationViewport 汇聚四态(conversation/tool-running/approval/failure 有界诊断不替换 dock);AgentApprovalCard 七要素如实显示+唯一主按钮「批准并提交」走真实 IPC(renderer→preload→main ai.agent.approval.respond once/reject);AgentToolActivityRow 默认单行折叠(§12.5);AgentSecondaryDrawer 模型服务/工具库存/历史/设置迁入(§12.10 组件树),关闭不取消 main-owned task;AgentScrollToBottom 按 60C scroll threshold(48px);ipc agent.approval.decide 通道(§12.11 DecideAgentApprovalRequest 消费点:decoder 校验+settleApproval+`expectedRevision` stable id) |
| `EV-REL-SCOPE-MATBIN-CLASSIFY-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-AGENT60D-TOOL-APPROVAL-DRAWER-20260814`;MATBIN 范围条目归类合法化:`SCOPE-MATBIN-53DE-DEFERRAL` `capabilityId` MATERIAL-53(§3.1 未定义)→E-ASSET、`subjectKind` material-format(枚举非法)→asset;`REL-SCOPE`.`scopeItemIds` 补引 `SCOPE-MATBIN-53DE-DEFERRAL` + `successorRequirement` 同步 28 项/13 项延期;重投影 §18.2.1;release-scope 门禁全绿,语义未变(仍 V0.6 延期只读,`resumeRequires` 不变),仅修归类合法性使门禁自洽 |
| `EV-VFX54C-GPARAM11B-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-REL-SCOPE-MATBIN-CLASSIFY-20260814`;VFX-54C FXR write + GPARAM-11B 五区返工:write-fxr-document(仅 vfx-field-set typed mutation,字节级外科替换,已知布局门 fail-closed,Section9/12-14 非空 block);`vfxBridgeCommit` + `runNativeFxrWriterSmoke`(合成 fixture 走真实 C# writer,5 类失败注入 + reopen-failure before-image);ipc `commitFxrFieldSet` + preload facade + BridgeDaemonHost/write-boundary/tiers/BACKUP 8 全接;GPARAM 四栏合并改五区 Files\|Groups\|Fields\|Values\|Toolbar(§8.1 逐字要求),Fields/Values 分离,单测+e2e 断言五区 |
| `EV-SHELL09-MAP50AB-20260814` | `scope-ruling:user-approved`;scope-deferral:`REL-E`:V0.6:user-approved;scope-deferral:`REL-I`:V0.6:user-approved;revalidates=`EV-VFX54C-GPARAM11B-20260814`;SHELL-09 Catalog 驱动 full-bleed shell + MAP-50A/B:`domainNavigation` 只消费 DomainSummary[] 固定顺序领域集合(project/files 常驻,其余按 read contract 门控),无 domainForFile/物理计数,顶部无 PARAM 36,editor full-bleed;pane 独立滚动;MAP-50A MSB read:`msbBridgeRead` + bridge read-msb-document,9 图 msb-all 全量 semantic roundtrip + type 注册表覆盖,`parseStatus` failed 与最小实体数 fail-closed;MAP-50B Smithbox Map 三栏工作台 Map Object List\|Viewport\|Properties,选择/线框高亮/属性联动,deferred 无写入口 |
| `EV-FMG-CONTAINER-CLOSED-LOOP-20260814` | `W-REL-C-MULTILANG-03`：全官方语言 FMG 容器闭环验证。`runNativeFmgSmoke` 主流程修复 `oodleRuntimeRoot` 缺失（此前 KRAK 报「尚未挂载 Sekiro 原版游戏目录」，仅全语言矩阵传过该参数），经 SOULFORGE_SEKIRO_GAME_ROOT 注入真实游戏根后主流程全链通过：item.msgbnd snapshot→native 读→语义往返→upsert/add/delete staged 写→独立重读→源文件哈希不变→BND4 容器 Patch Engine commit→提交后重读含变更→容器 rollback→corpus 18/18 FMG 语义往返；menu.msgbnd 15/15 读往返 + write-fmg upsert staged 写/重读/原 child 不变；enus 派生第二语言样本（upsert×3+add+delete，布局结构与 zhocn 不同）读往返/二次写/zhocn 源零污染；失败关闭 3 类（add 重复 id/非整数 id/越界 id）均 FMG_STAGING_WRITE_FAILED 且暂存输出不产生；14/14 官方语言真实 item.msgbnd FMG v2 条目 upsert staged 写/独立重读/原容器哈希逐项不变（每语言 semanticRoundTripVerified+stagedRereadVerified+originalContainerUntouched）；容器级引用完整性只读检查 item 18 fmg/menu 15 fmg 通过（`duplicateId` 如实诊断、dangling 标 warning 不声明外部语义）。`runFmgReferenceIntegritySmoke`（`testId` `W-EMEVD-FMG-PARAM-03-FMG-REF`）ok:true status partial（只读、零写路径）。authority 保持 partial。 |
| `EV-REL-D-GAMELOAD-PREFLIGHT-20260814` | `W-REL-D-GAMELOAD-01` 游戏加载 preflight 腿：script 容器整内层替换的游戏加载前置预检（synthetic 负向 7 case + 真实 aicommon.luabnd.dcx 301 条目）。`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-REL-SCOPE-GOVCLI-MULTIGATE-20260814` | revalidates=`EV-REL-SCOPE-MATBIN-CLASSIFY-20260814`;gov CLI fixture 修复:complete 成功路径构造后继切片时给所有引用目标切片的 open Gate 补后继,消除单 Gate 盲点(`REL-C`/`REL-D` 双挂 GAMELOAD-01 暴露:只给第一个 Gate 补后继导致 complete 在第二个 Gate 诚实拒绝,成功路径从未执行)。冻结范围语义未变。`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-MODLOADER-BAN-REVOKE-20260818` | revalidates=`EV-REL-SCOPE-GOVCLI-MULTIGATE-20260814`;2026-08-18 用户裁定撤销不自实现 Mod loader 取代 me3 禁令,主题域改动提交 `84f8f3ff`(`scope.json` `runtimeToolPolicy` 修订 + `SCOPE-RUNTIME` 移除 implement-mod-loader + `releases.json` `unfreezeRuling` 第三次追加 `SCOPE-RUNTIME` + AGENTS.md/交接书 §16 移除 + playbook K5 更新),handoff:project 重投影完成,冻结范围语义经 `unfreezeRuling` 记录;`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-MAP-ANIM-SCALE-20260823` | revalidates=`EV-MODLOADER-BAN-REVOKE-20260818`;地图与动作架构升级+启动器修复+层级补齐;`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-MAP-ANIM-SCALE-20260823-FINAL` | revalidates=`EV-MAP-ANIM-SCALE-20260823`;README 链接修复+全套治理与单元验证通过;`scope-ruling:user-approved` scope-deferral:`REL-E`:V0.6:user-approved scope-deferral:`REL-I`:V0.6:user-approved |
| `EV-ALIGN-MATURE-WORKBENCH-20260823-01` | Align Map Free-Look Fly camera, Model Resource Pool, FLVER skinning GPU attributes, and Event streaming workbench with mature tools; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; revalidates=`EV-MAP-ANIM-SCALE-20260823-FINAL` |
| `EV-V05-CONVERGENCE-FINAL-20260824` | revalidates=`EV-ALIGN-MATURE-WORKBENCH-20260823-01`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; Action Havok spline decompressor + companion ANIBND scoping, Event DarkScript compiler + native KRAK EMEVD verification, Map atomic multi-op transaction + SceneGraph split indices, Agent durable/ephemeral context separation + RRF hybrid retrieval. |
| `EV-V05-TIER-AUDIT-SEAL-20260824` | revalidates=`EV-V05-CONVERGENCE-FINAL-20260824`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; Register smoke scripts in `tiers.mjs` and packages, align §18.5 deferral index with shared protocol. |
| `EV-V05-UNFREEZE-SCOPE-SEAL-20260824` | revalidates=`EV-V05-TIER-AUDIT-SEAL-20260824`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; Support `unfreezeRuling` in verify-release-`scope.mjs` and pass full governance suite. |
| `EV-V05-TEMP-CLEANUP-SEAL-20260824` | revalidates=`EV-V05-UNFREEZE-SCOPE-SEAL-20260824`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; Clean up temp directory in `runNativeEmevdSmoke.ts` and pass `test:smoke-temp-cleanup`. |
| `EV-V05-TIER-NATIVE-SEAL-20260824` | revalidates=`EV-V05-TEMP-CLEANUP-SEAL-20260824`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; Assign `test:verify-map-action-open` to native tier in `tiers.mjs` and pass full governance tier. |
| `EV-MAIN-MERGE-CONVERGENCE-20260824` | revalidates=`EV-V05-TIER-NATIVE-SEAL-20260824`; `scope-ruling:user-approved`; scope-deferral:`REL-E`:V0.6:user-approved; scope-deferral:`REL-I`:V0.6:user-approved; Merge fix/v05-mature-cleanroom to main with full Action, Event, Map, Agent/RAG implementations and ThreeSceneController animation playback support. |
...
`W-REL-SCOPE-RULING-01` 的机器可读冻结产物是 `docs/governance/scope.json`（策略字段、全部 scopeItem、operations 与 unsupportedOperations 全文）与 `docs/governance/gates.json`（Gate 覆盖）。下表是它们的人读投影，由 `npm run handoff:project` 生成：一条 scopeItem 一行，operations 只给条数。要查某条具体支持哪些操作、非声明了什么、延期条目的 `resumeRequires` 写了什么，去 `scope.json`。

本块此前是 1467 行内嵌 JSON，逐字复制 `scope.json`。那份复制与权威长期分叉（27 条 scopeItem 全部缺 `targetRelease`、`deferredTrack`、`resumeRequires`，`schemaVersion` 停在 1.6.0 而权威已是 2.0.0），因为范围门禁只解析这个块、从不读 `scope.json`。现在 `node scripts/verify-release-scope.mjs` 直接读治理 JSON，本块只需与权威条目集一致——不一致由 `SCOPE_SUMMARY_ITEM_MISSING` / `SCOPE_SUMMARY_ITEM_UNKNOWN` 失败关闭。

`--proposal` 验证结构、引用和非声明完整性；默认严格模式还要求完整用户批准元数据、版本族策略、全部条目裁定和空 openRulings。开发期私有 fixture registry 仅用于复现当前证据，不是 release corpus，也不能替代后继发布 Gate 的真实验证。
...
|---|---|---|---|---|---|---|---|---|
| `SCOPE-SEKIRO-BUILD` | `H-RUNTIME` | `REL-SCOPE`（`passed`）、`REL-H`（`open`） | `supported` | `V0.5` | `unverified` | 4 | 3 | Sekiro 1.6.x 文件/产品版本族与 build identity |
| `SCOPE-A-WORKSPACE` | `A-WORKSPACE` | `REL-A`（`passed`） | `supported` | `V0.5` | `partial` | 5 | 4 | 单一可写 Mod 叠加层、可选只读原版层与应用数据外置 |
| `SCOPE-A-RECOVERY` | `A-RECOVERY` | `REL-A`（`passed`） | `supported` | `V0.5` | `partial` | 8 | 4 | operation、file、resource-entry 三层审计、回滚与崩溃恢复 |
| `SCOPE-DFLT` | `B-DFLT` | `REL-B`（`passed`） | `supported` | `V0.5` | `native-verified` | 5 | 2 | 注册 Sekiro DFLT 布局的解压、无修改往返、重压与重读 |
| `SCOPE-KRAK` | `B-KRAK` | `REL-B`（`passed`） | `supported` | `V0.5` | `partial` | 7 | 3 | 登记 Sekiro KRAK 布局与用户合法 Oodle runtime 的解压、重压、写回、重读与恢复闭环 |
| `SCOPE-BND4` | `B-BND4` | `REL-B`（`passed`） | `supported` | `V0.5` | `native-verified` | 11 | 3 | 注册 Sekiro BND4 布局的 browse、entry mutation、repack、重读与回滚 |
| `SCOPE-FMG` | `C-FMG` | `REL-C`（`open`） | `supported` | `V0.5` | `native-verified` | 10 | 3 | Sekiro 全部官方语言与登记 msgbnd/FMG v2 布局的完整文本读写 |
| `SCOPE-PARAM` | `C-PARAM` | `REL-C`（`open`） | `supported` | `V0.5` | `partial` | 13 | 9 | SoulForge first-party Sekiro 1.6.x PARAM schema 严格匹配下的 gameparam 全部 ParamType、布局、字段与行完整读写；drawparam / gparam 延期至 V0.6 |
| `SCOPE-EMEVD` | `C-EMEVD` | `REL-C`（`open`） | `supported` | `V0.5` | `partial` | 12 | 6 | Sekiro 1.6.x 在 SoulForge first-party schema 覆盖内的 EMEVD 事件、指令、控制流与参数无损读写；未覆盖及长度不匹配变体只读 |
| `SCOPE-MSB` | `C-MSB` | `REL-C`（`open`） | `supported` | `V0.6` | `native-verified` | 5 | 6 | Sekiro MSB 实体类型注册表的完整语义读取、注册表全覆盖与 typed mutation 写链验证（V0.6 承接交付；编辑器写入保持 `releaseWriteEnabled` 门控） |
| `SCOPE-BEHAVIOR-ANIMATION` | `D-BEHAVIOR` | `REL-D`（`open`） | `supported` | `V0.5` | `unverified` | 10 | 2 | 真实 Sekiro corpus 中 TAE 动画引用、ACTION 预览定位、动画读写与行为动画编辑开发：TAE animation → `motionAnimId` → 同 character family 的 ANIBND Binder entry，供 clip/pose 与编辑链使用。 |
| `SCOPE-BEHAVIOR-TAE` | `D-BEHAVIOR` | `REL-D`（`open`） | `supported` | `V0.5` | `candidate` | 8 | 1 | Sekiro TAE 文档的布局、事件、时间轴、参数投影、动画引用 identity 与读写/编辑开发；供 ACTION motion identity、clip/pose 和 release editor 使用。 |
| `SCOPE-BEHAVIOR-ESD` | `D-BEHAVIOR` | `REL-D`（`open`） | `supported` | `V0.5` | `candidate` | 9 | 2 | Sekiro ESD 状态机、条件表达式、命令、跳转关系的解析与读写/编辑开发（含转移边闭合图与悬空目标检出）；表达式与命令参数体的 RPN 字节码按不透明 (offset,length) 上报，后续语义与重编码继续由验证推进 |
| `SCOPE-BEHAVIOR-SCRIPT` | `D-BEHAVIOR` | `REL-D`（`open`） | `supported` | `V0.5` | `unverified` | 9 | 4 | 真实 Sekiro corpus 中 luabnd / action *.hks 脚本容器的条目枚举、只读字节码证据投影、经 Patch Engine 的整个内层文件替换/重读/回滚，以及**明文条目的源码级编辑**（容器内与 action 目录下少数以纯文本存放的脚本与符号表；字节码条目不在源码级编辑范围内） |
| `SCOPE-ASSETS` | `E-ASSET` | `REL-E`（`passed`） | `deferred` | `V0.5` → 延期 `V0.6` | `candidate` | 0 | 6 | 固定 FLVER/TPF/MTD/collision/navigation 五类 native 资产只读 authority 与 native-to-open 导出矩阵（延期至 V0.6） |
| `SCOPE-ASSET-FLVER` | `E-ASSET` | `REL-E`（`passed`） | `supported` | `V0.6` | `partial` | 5 | 6 | Sekiro 已登记 FLVER 布局的 geometry、skeleton、weights、dummy 与只读 native document（V0.6 承接交付） |
| `SCOPE-ASSET-TPF` | `E-ASSET` | `REL-E`（`passed`） | `supported` | `V0.6` | `native-verified` | 5 | 6 | Sekiro 已登记 TPF 布局的纹理格式、metadata 与只读 native document（V0.6 承接交付） |
| `SCOPE-ASSET-MTD` | `E-ASSET` | `REL-E`（`passed`） | `deferred` | `V0.5` → 延期 `V0.6` | `unverified` | 0 | 6 | Sekiro 全部 MTD 布局、材质参数、texture slot 与着色引用的只读文档（延期至 V0.6） |
| `SCOPE-ASSET-COLLISION` | `E-ASSET` | `REL-E`（`passed`） | `deferred` | `V0.5` → 延期 `V0.6` | `unverified` | 0 | 5 | 真实 Sekiro 中全部碰撞格式、层级与地图关联的只读语义文档（延期至 V0.6） |
| `SCOPE-ASSET-NAVIGATION` | `E-ASSET` | `REL-E`（`passed`） | `deferred` | `V0.5` → 延期 `V0.6` | `unverified` | 0 | 5 | 真实 Sekiro 中全部导航格式、连接关系与地图引用的只读语义文档（延期至 V0.6） |
...
| `SCOPE-EDITORS` | `F-EDITORS` | `REL-F`（`passed`） | `supported` | `V0.5` | `partial` | 12 | 5 | BND4、FMG、PARAM、EMEVD、TAE、ESD 与 script 编辑器能力的持续开发、typed mutation/整内层替换、共享文档与证据视图；MSB/FLVER 保留既有独立 typed write 路径；Hex 保持独立证据视图 |
...
| `SCOPE-V09-GITHUB-UPDATE` | `H-RUNTIME` | `REL-H`（`open`）、`REL-COMPLIANCE`（`passed`） | `supported` | `V0.9` | `unverified` | 9 | 9 | V0.9.2 Windows x64 GitHub Release 更新链：从固定 SoulForge Release 选择非降级预发布，要求 x64 NSIS 安装包、latest.yml、blockmap 与 SHA256SUMS 一致，下载完成后由用户确认并启动安装；v0.9.1 保持手动安装 bootstrap 边界 |
...
| `REL-D` | V0.5/V0.6 过渡期已解除 `SCOPE-BEHAVIOR-ANIMATION`、TAE、ESD 与 `SCOPE-EDITORS` 的版本性开发限制；ACTION/TAE motion identity、Binder membership、clip/pose、TAE/ESD writer、unknown-structure reencode 与 release editor 均可继续开发。实际 authority 仍按真实语料、native writer、Patch Engine、重读/恢复与功能验证分别建立。 |
...
| `REL-F` | 需交付当前 `scope.json` `editorIds` 中登记的语义编辑器、共享 Hex 证据视图、完整有界访问与 Electron 真实文档功能验收；V0.5/V0.6 过渡期不以版本性禁令阻断已登记编辑器的 writer 或 release-editor 开发；不要求量化容量或延迟门槛。 |
...
| `REL-C` | 核心语义 mutation 矩阵 | `W-REL-D-GAMELOAD-01`、`W-FIRSTPARTY-SCHEMA-01` | `open` | `in-scope` | `EV-EMEVD-FMG-PARAM-03-20260803`、`EV-FMG-MULTILANG-WRITE-20260804`、`EV-PARAM-CORPUS-138-20260804`、`EV-FMG-OFFICIAL-LANGUAGES-20260805`、`EV-FMG-CONTAINER-CLOSED-LOOP-20260814` | EMEVD DSL production Bridge/PatchIR transaction 与 138 ParamType corpus 已完成；FMG 已完成 14/14 官方语言真实 item staged 写入/重读。继续全语言 item/menu 容器级 Patch Engine 提交/独立重读/回滚与跨语言引用矩阵；完成后再单列真实 Sekiro 游戏加载确认 |
| `REL-D` | 行为动画范围 | `W-REL-D-GAMELOAD-01` | `open` | `in-scope` | `EV-BEHAVIOR-MAP-01-20260803`、`EV-SCRIPT-READONLY-01-20260803`、`EV-ESD-SUPPORTED-TRANSITION-EDGES-20260808`、`EV-SCRIPT-PLAINTEXT-SOURCE-EDIT-RULING-20260808`、`EV-REL-D-GAMELOAD-PREFLIGHT-20260814` | 本 Gate 继续保持 open；后继要求 ACTION motion identity → Binder membership → clip/pose IPC 的失败关闭、缓存失效与真实 Sekiro 语料验证，TAE/ESD writer 与 unknown-structure reencode 的 native/重读/恢复矩阵，script 容器闭环，以及 ESD 解析与编辑器功能闭环。范围放开不自动提升 authority 或 Gate 终态。 |
| `REL-E` | 资产只读与导出矩阵 | `W-FLVER-READ-01` | `passed` | `in-scope` | `EV-REL-SCOPE-20260731-TEXT-FIRST`、`EV-REL-SCOPE-20260801-GOVERNANCE-JSON`、`EV-REL-SCOPE-20260801-SEAL-CLI`、`EV-REL-SCOPE-20260801-HANDOFF-PROJECTION`、`EV-REL-SCOPE-20260801-SCOPE-PROJECTION`、`EV-REL-SCOPE-20260801-COMMAND-INDEX`、`EV-REL-SCOPE-20260801-PARSER-REGISTRY`、`EV-REL-SCOPE-20260801-DIAGNOSTIC-ROOTCAUSE`、`EV-REL-SCOPE-20260801-CLI-CLOSURE`、`EV-REL-SCOPE-20260801-RELEASE-SCOPING`、`EV-REL-SCOPE-20260801-EMPTY-GUIDANCE`、`EV-REL-SCOPE-20260801-COMMAND-EXISTENCE`、`EV-REL-SCOPE-20260802-SEAL-COMMIT-WARNING`、`EV-REL-SCOPE-20260802-UNCOMMITTED-PATH-FIX`、`EV-REL-SCOPE-20260802-DEFERRED-RESUME-PROJECTION`、`EV-REL-SCOPE-20260802-ENTRYPOINT-OPENABLE`、`EV-REL-SCOPE-20260802-CONSTRAINT-SPEC`、`EV-REL-SCOPE-20260802-STALE-CLAIM-VISIBILITY`、`EV-REL-SCOPE-20260802-FIXTURE-RESTORE`、`EV-REL-SCOPE-20260802-FIXTURE-PREMISE`、`EV-REL-SCOPE-20260802-EMPTY-CLAIM-STATE`、`EV-REL-SCOPE-20260802-STATUS-STALENESS`、`EV-REL-SCOPE-20260802-PROCESS-SELF-OPTIMIZATION`、`EV-REL-SCOPE-20260802-STATUS-FALSE-GREEN`、`EV-REL-SCOPE-20260802-EVIDENCE-INDEX-COMPACTION`、`EV-REL-SCOPE-20260802-MECH-TASKS-CLOSEOUT`、`EV-REL-SCOPE-20260802-PLAN-DIRECTORY-EXIT`、`EV-REL-SCOPE-20260803-HANDOFF-ENTRY-SECTION`、`EV-REL-SCOPE-20260803-V05-SLICES-BATCH`、`EV-REL-SCOPE-20260803-RENDERER-REMOVAL`、`EV-REL-SCOPE-20260803-RENDERER-RESTORE`、`EV-REL-SCOPE-20260804-SUBJECT-SET`、`EV-V06-RESUME-20260804`、`EV-REL-SCOPE-20260804-V06-FIXTURE-FIX`、`EV-REL-SCOPE-20260805-V06-RESUME-SCOPE-FIXTURES`、`EV-SECURITY-BOUNDARY-RUNTIME-20260807`、`EV-ROUNDTRIP-PAGINATION-20260807`、`EV-SKIP-SEMANTICS-TPF-TAE-20260807`、`EV-REBASE-REANCHOR-20260808`、`EV-CMDSET-STAGINGCODE-MTD-20260808`、`EV-GATE-BLINDSPOT-ESD-TPF-20260808`、`EV-BC7-PAGINATION-ESD-SENTINEL-20260808`、`EV-BC3-FLVERGAP-AIWIRE-NATIVEROOT-20260808`、`EV-T14-WIRING-COMPLETE-FLVER-PARTIAL-20260808`、`EV-PNGCOLORSPACE-DDSTRUNC-TFMGATE-20260808`、`EV-ESDGAP-XMACHINE-PARAMKRAK-20260808`、`EV-ESD-SUPPORTED-TRANSITION-EDGES-20260808`、`EV-SCRIPT-PLAINTEXT-SOURCE-EDIT-RULING-20260808`、`EV-AGENTIC-APPROVAL-WIRING-PLAINTEXT-20260808`、`EV-PENDING-RULINGS-RECORDED-20260808`、`EV-CODEX-DECISION-PARITY-20260808`、`EV-PLAINTEXT-RW-CHAIN-COMPLETE-20260809`、`EV-MIXED-ENCODING-BYTE-EDIT-20260809`、`EV-SMITHBOX-PARAM-FIELDS-EDITOR-LAYOUT-20260810`、`EV-ROOT07-BRIDGE-ROOTS-20260813`、`EV-PARAM-10A-PRIMARY-READ-20260813`、`EV-PARAM-10BC-SMITHBOX-FOURCOLUMN-WRITEBACK-20260813`、`EV-GPARAM-11A-NATIVE-READ-20260813`、`EV-GPARAM-11B-WORKBENCH-20260813`、`EV-GPARAM-11C-WRITEBACK-20260813`、`EV-MODEL51-TEXTURE52-20260813`、`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`、`EV-VFX54-FXR-READ-20260814`、`EV-MATBIN-53DE-DEFERRED-20260814`、`EV-MATERIAL53C-WRITEBACK-20260814`、`EV-MAP50C-MSB-WRITEBACK-20260814`、`EV-VFX54B-WORKBENCH-20260814`、`EV-MATERIAL53B-BEHAVIOR55B-ANIMATION56B-WORKBENCH-20260814`、`EV-AGENT60A-DOCK-SHELL-20260814`、`EV-BEHAVIOR55C-ESD-WRITEBACK-20260814`、`EV-AGENT60B-COMPOSER-20260814`、`EV-ANIMATION56C-TAE-WRITEBACK-20260814`、`EV-AGENT60C-SELECTION-STREAM-20260814`、`EV-AGENT60D-TOOL-APPROVAL-DRAWER-20260814`、`EV-REL-SCOPE-MATBIN-CLASSIFY-20260814`、`EV-VFX54C-GPARAM11B-20260814`、`EV-MODLOADER-BAN-REVOKE-20260818`、`EV-MAP-ANIM-SCALE-20260823`、`EV-MAP-ANIM-SCALE-20260823-FINAL`、`EV-V05-CONVERGENCE-FINAL-20260824`、`EV-FLASH-AUDIT-REVALIDATE-20260907`、`EV-REL-V091-NSIS-SCOPE-20260913` | 资产线 V0.6 部分已交付：FLVER/TPF 只读 authority 在登记样本上验证完成（`native-verified`）；5 项范围条目仍保持 deferred，后继里程碑继续 MTD 语义读取 authority、collision（.clm2 in-BND4）/navigation 格式解析、完整引用对齐与 native-to-open 导出闭环；既有 deferred 预览保持只读。 |
| `REL-F` | 编辑器验收 | `W-REL-F-ACCEPT-02` | `passed` | `in-scope` | `EV-REL-F-ACCEPT-02-20260803`、`EV-REL-SCOPE-20260803-RENDERER-REMOVAL`、`EV-NAMING-HONESTY-ROUNDTRIP-20260807`、`EV-REBASE-REANCHOR-20260808`、`EV-WORKBENCH-PARAM-TRUST-20260810`、`EV-CONTAINER-PARAM-WRITEBACK-20260810`、`EV-MODLOADER-BAN-REVOKE-20260818`、`EV-MAP-ANIM-SCALE-20260823`、`EV-MAP-ANIM-SCALE-20260823-FINAL`、`EV-FLASH-AUDIT-REVALIDATE-20260907` | inventory 以 `scope.json` 的 `editorIds` 与 `editorMutationModes` 为准，当前覆盖 BND4/FMG/PARAM/EMEVD/script/MSB/TAE/ESD/FLVER；继续各编辑器结构化 UI/DSL、完整有界访问、native writer/重读/恢复与 Electron 真实文档功能验收。范围开放不等于功能或 Gate 已完成，Hex 证据视图仍按其独立契约处理 |
| `REL-G` | 双协议 AI | `W-AI-CONFORMANCE-03` | `passed` | `in-scope` | `EV-AI-CONFORMANCE-03-20260803`、`EV-AI-CODEX-KERNEL-20260804`、`EV-AI-CODEX-PRODUCTION-20260804`、`EV-AI-CODEX-SUBJECT-SET-20260804`、`EV-TPF-SCAFFOLD-CI-20260806`、`EV-RECOVERY-AIPERM-COVERAGE-20260807`、`EV-BC7-PAGINATION-ESD-SENTINEL-20260808`、`EV-BC3-FLVERGAP-AIWIRE-NATIVEROOT-20260808`、`EV-T14-WIRING-COMPLETE-FLVER-PARTIAL-20260808`、`EV-AGENT-TOOL-SCHEMA-EXPOSURE-20260808`、`EV-AGENTIC-APPROVAL-WIRING-PLAINTEXT-20260808`、`EV-PERMISSION-JUDGE-UNIFIED-20260808`、`EV-APPROVAL-UNIFIED-DIFF-20260808`、`EV-CODEX-DECISION-PARITY-20260808`、`EV-PLAINTEXT-RW-CHAIN-COMPLETE-20260809`、`EV-SMITHBOX-PARAM-FIELDS-EDITOR-LAYOUT-20260810`、`EV-WORKBENCH-PARAM-TRUST-20260810`、`EV-CONTAINER-PARAM-WRITEBACK-20260810`、`EV-WORKBENCH-EDITOR-SELECTION-RECENTPATHS-20260810`、`EV-ROOT07-BRIDGE-ROOTS-20260813`、`EV-PARAM-10A-PRIMARY-READ-20260813`、`EV-PARAM-10BC-SMITHBOX-FOURCOLUMN-WRITEBACK-20260813`、`EV-GPARAM-11A-NATIVE-READ-20260813`、`EV-GPARAM-11B-WORKBENCH-20260813`、`EV-GPARAM-11C-WRITEBACK-20260813`、`EV-MODEL51-TEXTURE52-20260813`、`EV-MATERIAL53-BEHAVIOR55-ANIMATION56-READ-20260814`、`EV-VFX54-FXR-READ-20260814`、`EV-MATERIAL53C-WRITEBACK-20260814`、`EV-MAP50C-MSB-WRITEBACK-20260814`、`EV-VFX54B-WORKBENCH-20260814`、`EV-MATERIAL53B-BEHAVIOR55B-ANIMATION56B-WORKBENCH-20260814`、`EV-AGENT60A-DOCK-SHELL-20260814`、`EV-BEHAVIOR55C-ESD-WRITEBACK-20260814`、`EV-AGENT60B-COMPOSER-20260814`、`EV-ANIMATION56C-TAE-WRITEBACK-20260814`、`EV-AGENT60C-SELECTION-STREAM-20260814`、`EV-AGENT60D-TOOL-APPROVAL-DRAWER-20260814`、`EV-VFX54C-GPARAM11B-20260814`、`EV-MODLOADER-BAN-REVOKE-20260818`、`EV-MAP-ANIM-SCALE-20260823`、`EV-MAP-ANIM-SCALE-20260823-FINAL`、`EV-V05-CONVERGENCE-FINAL-20260824`、`EV-MISSION1-REVALIDATE-20260825`、`EV-FLASH-AUDIT-REVALIDATE-20260907`、`EV-REL-V091-NSIS-SCOPE-20260913` | 错误/取消/超时/限额 10 case 已完成；继续真实工作区多步 typed mutation 矩阵 |
...
| `BLK-BEHAVIOR-CORPUS` | `private-corpus` | 历史：`REL-D`、`W-BEHAVIOR-MAP-01`；当前无活动引用 | 历史记录；当前由工程方推进 | 已解除用户输入：本机 registry 已支持登记 TAE/ESD native document，并观察到 HKX/Lua 条目 | TAE/ESD 已解除版本性开发限制；当前继续推进 motion identity、transition、writer 与 editor 链，完整语义与 release authority 仍须由真实 corpus、native writer、Patch Engine、重读/恢复和功能验证分别建立 | 本机 corpus 被撤回、新增声明变体，或真实验证边界发生变化时重新打开 | `EV-OWNER-INPUTS-IMPLEMENTATION-20260730`、`EV-REL-SCOPE-20260731-TEXT-FIRST` |
