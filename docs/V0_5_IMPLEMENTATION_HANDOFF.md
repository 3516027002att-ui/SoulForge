...
| `W-MSB-SCENE-01` | `deferred` | `partial` | — | `C-MSB` / `I-RENDER` | MSB 与渲染线延期 V0.6；本切片成果保留为只读预览，releaseWriteEnabled=false 必须保持关闭｜已完成证据：已建立 shared schema v2 semantic scene/render packet、四类 Bridge preview、稳定 identity/revision、chunk、路径防线和 production canvas/picking，且 msb_set_part_transform typed mutation 曾经真实 MSB 验证通过；文本优先裁定后 MSB 与渲染线延期 V0.6，本切片成果保留为标记只读预览，releaseWriteEnabled=false 关闭写路径 | entity identity 与 revision 稳定；renderer 无绝对路径；延期期间不得开放任何 MSB 写入（contract / shared 清单 / 主进程 IPC 三层失败关闭）；已验证的 `mutationKinds` 记录不删除，V0.6 恢复只需翻回标记 | `packages/shared/src/scene-ir.ts`、`packages/core/src/editing/msbBridgeRead.ts` | `npm run bridge:verify:msb`、`npm run test:scene-draw-list`、`npm run test:three-scene-module` | cap=`partial`；完整实体流式投影、writer、FLVER 或游戏加载进入后继切片 |
...
|---|---|---|---|---|---|
| `W-SCRIPT-READONLY-01` | `claim-w-script-readonly-01-20260803` | main-agent | 2026-08-03T01:43:15.185Z | 2026-08-03T01:43:15.185Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-REL-F-ACCEPT-02` | `claim-w-rel-f-accept-02-20260803` | main-agent | 2026-08-03T01:43:15.627Z | 2026-08-03T01:43:15.627Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-A-RECOVERY-INTEGRATION-04` | `claim-w-a-recovery-integration-04-20260803` | main-agent | 2026-08-03T01:43:16.111Z | 2026-08-03T01:43:16.111Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-REL-B-CORPUS-02` | `claim-w-rel-b-corpus-02-20260803` | main-agent | 2026-08-03T01:43:16.618Z | 2026-08-03T01:43:16.618Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-EMEVD-FMG-PARAM-03` | `claim-w-emevd-fmg-param-03-20260803` | main-agent | 2026-08-03T01:43:17.148Z | 2026-08-03T01:43:17.148Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-AI-CONFORMANCE-03` | `claim-w-ai-conformance-03-20260803` | main-agent | 2026-08-03T01:43:17.605Z | 2026-08-03T01:43:17.605Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-ME3-INSTALL-04` | `claim-w-me3-install-04-20260803` | main-agent | 2026-08-03T01:43:18.062Z | 2026-08-03T01:43:18.062Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
| `W-REL-COMPLIANCE-02` | `claim-w-rel-compliance-02-20260803` | main-agent | 2026-08-03T01:43:18.545Z | 2026-08-03T01:43:18.545Z | 检查该切片 `entryPoints` 对应工作树改动与运行中的写进程；无相关写进程则保存后审查再原子回退 ready |
...
- `W-REL-F-ACCEPT-02`：Electron 真实文档功能验收（opt-in）与各编辑器结构化 UI/DSL 收尾；
...
全部 126 条已登记验证命令按层级列出。层级顺序即执行顺序（先快后慢，早失败早停）。
