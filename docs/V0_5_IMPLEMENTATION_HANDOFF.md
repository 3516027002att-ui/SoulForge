  ...
  "scopeItems": [
    ...
    },
    {
      ...
      "nonClaims": [
        ...
        "`801000_battle.lua` 是**混合编码**，不接受源码级编辑：原版日文 + 后来加的 GBK 中文注释（偏移 107502 的字节 `0xc9 0xef` 无法按 Shift-JIS 解码，整段 `ÓÒÊÖµ¯·ÉïÚ` 是 GBK 的「右手弹飞镖」，显然是中文 mod 作者改过）。任何单一编码的解码-编码往返都会丢字节。此前 detectPlaintextEncoding 把「不是 UTF-8」当成「就是 Shift-JIS」，会把它误判成可处理；已新增 mixed-unknown 取值并一律以 PLAINTEXT_MIXED_ENCODING_UNSUPPORTED 拒绝——误判的代价不是标签错了，是让拒绝理由说了假话。需要改动它时走整文件替换。",
        "端到端读写链路已在真实语料上验证 6/6（test:plaintext-script-write）：5 个明文条目（goal_list.lua、543000_battle.lua 两个容器内条目 + 三个 *nameid.txt 裸文件）各自走完「明文判定 → 源码级编辑 → Patch Engine 提交 → Bridge/磁盘重读一致 → 回滚逐字节还原」，另加 1 条经生产 AI 工具（propose_plaintext_script_edit）的同链验证。容器内条目走 container_child_replace，裸文件走 file_replace——两条写路径不同，各自验过；「容器条目通过」推不出「裸文件也通过」。",
