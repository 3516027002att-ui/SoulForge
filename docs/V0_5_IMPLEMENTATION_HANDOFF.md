...
| `EV-REL-SCOPE-20260802-STATUS-FALSE-GREEN` | `sealed-current-run` | `scope-ruling:user-approved`；scope-deferral:`REL-E`:V0.6:user-approved；scope-deferral:`REL-I`:V0.6:user-approved；revalidates=`EV-REL-SCOPE-20260802-PROCESS-SELF-OPTIMIZATION`；工程侧重封存：plan 按难度拆为 `docs/plan/HARD.md` 与 MECH.md 并解除用完即弃文件的硬依赖（PLAN_MISSING 移除、任务清单改为 MANAGED_DOCS 可选成员、条目交叉引用门禁改判任务 ID）；修复 gov status 的假绿（漏传 `withFreshness` 导致跳过 stale 判定，顶层 ok 与退出码不随门禁转红）。范围裁定语义未变，不构成新的范围主张。 | HEAD=`70a991623d67c51d18faa37447b2685ae72c7fab`; trackedDiffSha256=e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855; untrackedManifestSha256=e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855; handoffSha256BeforeEvidenceAppend=e418027dce0670398530143d69a811812703a3f7543617d8bdd5d446a4724a12; fingerprintSha256=3133324c5fe6b9fd3fd5204317bcc99c0e86932f0b3c93a2d71cbee2bcbdee38 | `node scripts/verify-handoff-integrity.mjs`（负向×3：追加 T-M9 → PLAN_TASK_REF_DANGLING；## T- → ## XT- → PLAN_TASK_IDS_UNPARSEABLE；删除 `docs/plan`/ → 仅 2 条 DEAD_LINK 无 PLAN_MISSING，均已还原）；`npm run test:gov-cli` 退出码 0，checks 89→92、`checkedRules` 24→25（负向×2：去掉 `withFreshness` → status-reports-stale-gate + status-exit-tracks-gate 转红；只去掉 ok/`exitCode` 两行 → 仅 status-exit-tracks-gate 转红）；`npm run typecheck` 退出码 0；`node scripts/verify.mjs` --tier governance 封存前退出码 1（仅三条 GATE_*_STALE） | 拆分与门禁改判完成；status 与 verify --tier governance 对同一状态给出一致结论（实测同状态下旧写法 governanceGateOk:true / 新写法 false）；两处修复各自独立被负向用例覆盖 | 不提升任何 native authority；不改变范围裁定与延期语义；不推送 origin；未运行 native smoke 与 `npm run build` |
