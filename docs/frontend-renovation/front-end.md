...
- **S29 二轮（2026-08-17 晚，裸 param / 纯文本出审查队列）**：① 裸 `.param` 行增删/复制（`ParamTablePanel.onMutation`）与字段写入（`ParamDefPanel.onApplyFieldMutation`）不再 `changeStore.propose`，直接 `applyParamMutation` / `applyParamFieldMutation` → Patch Engine，成功 toast「已保存」并重读；main 侧两条裸 param 通道补「哈希缺失现算」兜底（`expectedHash || file.sha256 || sha256FileNow`），现算值同时喂 Bridge `expectedDocumentHash`。② 通用纯文本 `saveCurrentText` 同样直写（按钮「生成变更候选」→「保存」），底层 `saveTextResource` → Patch Engine 照旧。③ 合并漂移修复：`ParamWorkbench` 的打勾判定改回共享 `isParamCheckboxField`（删掉本地重复 `isBoolLike` 实现）；脚本读取反编译条目的 `encoding` 恢复 `'decompiled'` 标记（保存链按 UTF-8 明文落盘不变）；TaeWorkbenchPanel 预览宿主补回 `tae-preview-host` class。审查队列此后只留给真·批量补丁。
...
- **S30 二轮（2026-08-17 晚，「N 槽 · M 有字」）**：Bridge `read-text-catalog` 的表元数据新增 `filledCount`（非空文本条数，读取时本来就要逐表解析 FMG，零额外 IO）；main 目录链透传；FmgWorkbenchPanel 左栏 meta / Entries hint / 工具条状态句统一 `formatSlotCount`：有 filledCount 时显示「N 槽 · M 有字」，旧 Bridge 缺字段时回落「N 条」。地名表因此直接显示「47 槽 · 6 有字」，不再像解析失败。单测同步：空态断言从已删除的「这张表没有已填写的条目」改为钉「当前页无条目。」+ 旧文案不再出现。
