...
上面三条讲怎么把门禁加对,这一节讲**别加**。两者同等重要——本仓库当前已有
**29 个 `scripts/verify-*.mjs`、50 条被调度的验证、17 个错误码、92 项 gov-cli 断言**,
`governance` 层单跑约 **37 秒**(均 2026-08-02 实测)。这套东西已经够密了,再加的边际
收益迅速下降,而每一条都要人读、要维护、要在改动时同步。

**先判断该不该有门禁,再判断怎么加对。** 顺序反了就会写出一堆"加对了但不该加"的东西。

### 六条不该加的形态

1. **已有上游判据能覆盖的。** 治理 JSON 有 schema 校验(`GOVERNANCE_SCHEMA_VIOLATION`),
   `MANAGED_DOCS` 有三处扫描,`tiers.mjs` 有 `--audit` 登记核对。加之前先跑一遍看
   现有门禁是不是已经会红。**重复覆盖不是双保险,是两处判据将来会分叉。**
2. **防的失败没真实发生过、也没有具体触发路径。** 判据是"能说出一个具体的人在什么情况下
   会怎么做错"。说不出来就别加。本档每条任务都带"实测依据"不是格式要求——**没有实测
   依据的门禁不该存在。**
3. **只有 agent 自己会犯、且犯了立刻就知道的。** 语法错、拼写错、路径写错这类由
   `typecheck`/`build`/现有链接门禁当场抓住的,不需要专门的门禁。
4. **判据比被检对象更容易错的。** 若门禁本身需要复杂正则、多文件交叉状态、
   或依赖面板状态才能判断,它的误红概率可能高于它防的失败概率。误红比漏检更贵——
   **指向错误原因的诊断比没有诊断更糟。**
5. **为通用性而加的参数、开关、可配置项。** 只有一个调用点就写死;`{ withFreshness = false }`
   这类默认关闭的开关已经造成过一次假绿(见 T-M2)。**默认值是负债。**
6. **对文档/注释措辞的门禁。** 措辞得人读。机械匹配措辞会在正常改写时误红,
   然后下一个人为了绕过它而扭曲行文。

### 加之前必须能回答的三个问题

- 它防的失败**实际发生过吗**?发生在哪次提交、什么症状?
- 现有的 29 个校验脚本里,**有没有一个已经会红**?(跑一遍,别猜)
- 它误红时,诊断会**指向真实原因还是指向症状**?

三个问题里有一个答不上来,**结论就是不加**。把判断过程写进任务条目再删除,
比留下一个没人敢动的门禁好。

### "不做"是合格交付

T-M3、T-H3 都明确写了"不做也是允许的结论"。**这适用于全部任务。**
本档任务的目标是让下一个 agent 少踩坑,不是让检查数变多。删掉一条任务并写明
"核实后判定不需要,理由是 X",与实现它同等合格。
...
**注意:** 探针本身是合法工具(定位断言是否真的执行时,插桩 `scripts/_probe*.mjs`
打印分支与 payload 是既有做法),门禁要拦的是
