...
**每轮推进都要顺手改掉一处让你自己变慢的东西,并把它固化成机制。**

前八条约束全部来自这个动作。它们不是设计出来的,是"agent 撞墙 → 定位根因 → 固化成
门禁或 CLI 输出"这个循环的产物。停掉这个循环,本文就会退化成一份没人维护的规则清单。

### 9.1 什么算摩擦

按"是否会静默重复发生"筛,不按主观烦躁程度筛:

- **你查了两遍以上的东西** → 它该出现在 `gov next` / `gov help` 的输出里。
  实测例:deferred 切片的承接条件要手工按 `capabilityId` 反查 `scope.json`,V0.6 的 3 条
  切片对应 12 个 scopeItem——每个 agent 都得重做一遍。现在直接投影。
- **你照诊断修但没修好的东西** → 诊断指向了错误原因,按第 4 条修根因。
- **你靠记忆而非工具避开的坑** → 下一个 agent 没有你的记忆。要么加门禁,要么写进本文。
- **两种状态输出相同但结论相反** → 见第 2 条推论二,agent 会稳定地选错那一边。
- **纯约定,没有门禁守着** → 迟早失效。实测例:本文第 6 条写了探针"用完删",
  但探针全部 gitignored 不进 `git status`,于是从 7-18 攒到 8-02 共 16 个才被发现。

### 9.2 固化的优先级

同一个摩擦,越靠前的修法越可靠:

1. **让错误不可能发生**——改数据结构或接口,使坏状态无法表达。
2. **让错误必然被发现**——加门禁,并按第 5 条用负例证明它会红。
3. **让错误自带修法**——诊断里给出可直接执行的下一步(第 4 条)。
4. **写进本文**——只有前三条都不适用时才用。散文没有执行力,会与代码分叉。

**不要跳到第 4 步。** 写进文档最省事,也最容易变成下一轮的技术债:文档不会因为代码改了
而报错,而"文档说 A、代码做 B"这种分叉恰恰无人发现——本仓库的假门禁形态之一
(第 5 条"只读副本")就是这么来的。
...
- 加了断言 → **检查总数必须涨预期条数**。不涨就是没执行(第 6 条)。
- 改了诊断 → 在**实测的失败场景**下读一遍,确认它指的是真实原因。
- 加了投影 → 量首屏体积,别为省一次检索换来每次都读的膨胀。
- 声称"更快了" → 给出改前改后的实测值,不用"应该会快"这类措辞(第 8 条)。

### 9.4 边界

- 优化流程**不得放宽验证、authority 或范围口径**。跑得更快不能靠少验证换。
- 不为了减少步骤而让 CLI 代替用户裁定。`user-approved` 标记永远不能由 CLI 自动补(第 7 条)。
- 一次只改一处摩擦并各自封存。多处混在一轮里,负例失败时无法定位是哪处引起的。
