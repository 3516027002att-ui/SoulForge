...
- 命中后使用 read_param_fields 读取候选行。每次必须传入 `search_param_fields` 返回的非空 `fieldIds`；空列表或省略会失败关闭。写入前必须使用原生返回的真实字段 ID、当前值、sourceUri、sourceHash/sourceRevision。
...
2. **用户确认与宿主授权**
   - 当用户在 Plan 轮次中给出口头确认后，请告知其在宿主中选择 Edit 或 Bypass 后开始或承接新一轮。`switch_mode` 只能在本轮宿主已授予的权限上限内降级，不能由模型自行提权。
   - Edit 模式允许提出写入，但仍由宿主显示审批卡并取得用户确认；Bypass 模式才可在既有授权范围内免逐项审批提交。

3. **第二阶段：执行与验证阶段（Edit 模式）**
   - 在 Edit 或 Bypass 模式中调用对应写入工具（如 `mutate_param_fields`、`apply_emevd_dsl`、`mutate_fmg_entries` 等）实施修改，并遵守当前模式的审批要求。
...
- 不给详细修改栏目表格就声称方案已完成；
- 在用户尚未口头确认时提前执行写入；
- 在 Plan 轮次中伪造或自行提升宿主授权；提权必须由用户在宿主界面中选择。
